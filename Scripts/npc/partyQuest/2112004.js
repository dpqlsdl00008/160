...
var mapID = [926100000, 926100001, 926100100, 926100200, 926100201, 926100202, 926100203, 926100300, 926100301, 926100302, 926100303, 926100304, 926100400, 926100401, 926100500, 926100600, 926100700];
...
function action(mode, type, selection) {
    ...
    switch (status) {
        ...
        case 1: {
            ...
            switch (selection) {
                case 0: {
                    cm.sendNext("\r\n저, 로미오와 줄리엣은 서로 사랑하는 사이입니다. 하지만 저는 제뉴미스트, 줄리엣은 알카드노 소속이기 때문에 서로 이루어질 수 없는 운명이에요...");
                    break;
                }
                case 1: {
                    cm.dispose();
                    ...
                    cm.resetMap(926100000);
                    cm.resetMap(926100001);
                    cm.resetMap(926100100);
                    cm.resetMap(926100200);
                    cm.resetMap(926100300);
                    cm.resetMap(926100301);
                    cm.resetMap(926100302);
                    cm.resetMap(926100303);
                    cm.resetMap(926100304);
                    cm.resetMap(926100400);
                    cm.resetMap(926100401);
                    cm.resetMap(926100500);
                    cm.resetMap(926100600);
                    cm.resetMap(926100700);
                    cm.getMap(926100401).spawnNpc(2112010, new java.awt.Point(250, 150));
                    cm.warpParty(926100000);
                    break;
                }
                case 2: {
                    if (cm.haveItem(4001159, 2) == false) {
                        cm.dispose();
                        cm.sendNext("\r\n로미오의 펜던트를 만들기 위해서는 제뉴미스트의 구슬 2개가 필요해요. 이걸로는 부족한 걸요?");
                        return;
                    }
                    cm.dispose();
                    cm.askAcceptDecline("#b제뉴미스트의 구슬 2개#k를 가지고 오셨군요. #i1122116# #b#z1122116##k를 만들 수 있어요. 지금 만들어 드릴까요?");
                    break;
                }
                case 3: {
                    ...
                    cm.dispose();
                    cm.askAcceptDecline("제 #b로미오의 펜던트#k 뿐만 아니라, 줄리엣의 #b줄리엣의 펜던트#k도 가지고 오셨군요. 이 두 목걸이를 하나로 합쳐 #i1122118# #b#z1122118##k를 만들 수 있어요. 지금 만들어 드릴까요?");
                    break;
                }
                case 4: {
                    cm.dispose();
        ...
        case 3: {
            ...
            switch (a2) {
                case 0: {
                    cm.sendNextPrev("\r\n저는 익명의 제보를 통해, 이것이 #b제 3의 인물#k의 짓임을 알게 되었습니다. 마가티아의 전쟁을 막고 줄리엣과의 사랑을 이루기 위해서는 #b제 3의 인물#k을 찾아 내, 그의 음모를 막아야만 합니다.");
                    break;
                }
            }
            break;
        ...
        case 4: {
            switch (a3) {
                case 0: {
                    ...
                    say += "\r\n#e- 제한 시간#n : 30분";
                    say += "\r\n#e- 참가 인원#n : 4명";
                    say += "\r\n#e- 획득 아이템#n : #i1122116# #d#z1122116##k";
        ...
        case 7: {
            cm.sendNextPrev("\r\n그럼 여러분께 제가 간직하던 #b제뉴미스트의 구슬#k과 함께 작은 보답을 드리고 밖으로 내보내 드리겠습니다.");
            break;
        }
        case 8: {
            cm.dispose();
            //Packages.handling.channel.handler.DueyHandler.addNewItemToDb(4001159, 2, cm.getPlayer().getId(), "로미오와 줄리엣", "", true);
            Packages.handling.channel.handler.DueyHandler.addNewItemToDb(4310050, 1, cm.getPlayer().getId(), "로미오와 줄리엣", "", true);
