...
function action(mode, type, selection) {
	...
	var t_2 = selection == 0 ? 4001514 : selection == 1 ? 4001516 : selection == 2 ? 4001522 : 0
		...
		cm.sendGetNumber("#b#z"+t_5+"##k를 구입하고 싶은거야? 한 장당 #b"+t_6/10000+"만 메소#k에 판매하고 있어. 몇 장 구입할래?",1,1,cm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.ETC).getNumFreeSlot()*100);
		...
		if(collab == 1) {
			...
			cm.sendOk("#b#z"+spirit+"# "+buyitem+"개#k는 잘 발았지? 그럼 몬스터파크에서 즐거운 시간 보내도록 해~ 후후후.");
			cm.dispose();
			}
		}
...
//			return;
				...
				if (cm.getPlayer().getMeso() < meso * selection){
					...
					cm.sendOk("#b#z"+ticket+"# "+selection+"개#k는 잘 발았지? 그럼 몬스터파크에서 즐거운 시간 보내도록 해~ 후후후.");
