...
function setup(eim, leaderid) {
    ...
    var eim = em.newInstance("expedition_vonleon" + leaderid);
    eim.setInstanceMap((parseInt(em.getProperty("map")))).resetFully();
    eim.setInstanceMap((parseInt(em.getProperty("map")) + 1)).resetFully();
    eim.setInstanceMap(parseInt(em.getProperty("map"))).spawnNpc(2161000, new java.awt.Point(-49, -181));
...
function changedMap(eim, player, mapid) {
    switch (mapid) {
        ...
        case (parseInt(em.getProperty("map")) + 1):
            return;
    }
