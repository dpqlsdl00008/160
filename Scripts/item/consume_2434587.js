...
1432167, // 파프니르 브류나크
1442223, // 파프니르 문 글레이브
1542063, // 파프니르 황도 마사무네

1372177, // 파프니르 마나 테이커
1382208, // 파프니르 마나 크라운
1552063, // 파프니르 황선 아이히메
1252015, // 파프니르 폴라리스 스틱

1452205, // 파프니르 윈드 체이서
1462193, // 파프니르 윈드 윙 슈터
1522094, // 파프니르 듀얼 윈드 윙
...
function action(mode, type, selection) {
    ...
    switch (status) {
        case 0: {
            if (cm.itemQuantity(2434587) < 30) {
                cm.dispose();
                return;
            }
            var say = "　 #Cgray#현재 직업이 착용 가능 한 장비를 우선 추천해 드립니다.\r\n　 받으실 무기를 선택해 주세요.";
            if (aList) {
                say = "　 #Cgray#받으실 무기를 선택해 주세요.";
            }
            ...
            for (i = 0; i < nEqp.length; i++) {
               ...
               if (!aList && v2 && (i < 9 || i > 12)) {
                   continue;
               }
               if (!aList && v3 && (i < 13 || i > 15)) {
                   continue;
               }
               if (!aList && v4 && (i < 16 || i > 19)) {
                   continue;
               }
               if (!aList && v5 && (i < 20)) {
                   continue;
