...
function action(mode, type, selection) {
    ...
    if (mData != null) {
        ...
        while (it.hasNext()) {
            ...
            while (v2.hasNext()) {
                ...
                if (maxLv > 30) {
                    if (cm.getPlayer().getMasterLevel(v3.getName()) < 30) {
    ...
    switch (status) {
        ...
        case 1: {
            cm.dispose();
            if (cm.getPlayer().getTotalSkillLevel(canSkill[selection]) < 15) {
                cm.sendNextS("아직 마스터리 북을 사용하기엔 자네가 고른 스킬의 레벨이 낮네. 해당 스킬의 레벨을 15 이상 올려 주게.", 4);
                return;
            }
            cm.gainItem(2431935, -1);
            cm.teachSkill(canSkill[selection], cm.getPlayer().getTotalSkillLevel(canSkill[selection]), 30);
