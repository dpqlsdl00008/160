...
function start(mode, type, selection) {
	...
	if (status == 0) {
		...
		qm.askAcceptDecline("그런데 두 개의 시계탑 중 한 곳이 어떤 이유에서인지 망가진 모양이야. 덕분에 #b루디브리엄에서 관리하는 시간에 구멍이 뚫려서 과거로 갈 수 있게 되었다#k더군... 재미있는 건 여기부터야.");
	} else if (status == 4) {
		...
		qm.sendPrev("망가진 시계탑은 #b오른쪽에 있는 시계탑#k... 즉, 핼리오스탑이야. #b분홍색 토끼 머리 모양 건물#k 안에 시간 관리 장치가 있는데, 그리로 들어가려면 #b핼리오스탑 최상층에서 사다리를 타고 위쪽으로 올라가#k면 된다고 해. 거길 통과해서 과거로 갈 수 있을거야.");
	} else if (status == 7) {
		qm.dispose();
	}
}
