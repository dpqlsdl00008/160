...
function start(mode, type, selection) {
    if (mode == 1) {
        ...
        if (status == 2) {
            qm.sendNext("일반공격도 물론 필요하지만 사냥의 기본은 역시 스킬이지요.");
            qm.dispose();
            return;
        } else {
            status--;
    ...
    switch (status) {
        ...
        case 2: {
            qm.sendAcceptDecline("자, 그럼 까먹기 전에 바로 실전연습입니다. 이 주변에는 티브들이 많이 있는데요, #b달팽이 세마리 스킬을 사용해 #r티브 3마리#k를 사냥한 후 그 증거로 #b티브의 깃털#k을 1개 가져와 주세요. 그럼 기다리겠습니다.");
            break;
        }
        case 3: {
            qm.dispose();
...
function end(mode, type, selection) {
    ...
    switch (status) {
        case 0: {
            qm.sendNext("티브를 퇴치하고 티브의 깃털을 가져오셨군요. 훌륭합니다. #b정식으로 기사가 되면 레벨업을 할 때마다 스킬 포인트를 3씩#k 얻으실 수 있습니다. 그럼, 화살표를 따라 왼쪽으로 쭈욱 가면 다음 단계를 담당하고 있는 #b키아#k를 만나실 수 있을 겁니다.\r\n\r\n#fUI/UIWindow.img/QuestIcon/4/0#\r\n#fUI/UIWindow.img/QuestIcon/8/0# 40 exp");
            break;
        }
        case 1: {
            qm.dispose();
