...
function start(mode, type, selection) {
    if (mode == 1) {
        ...
        if (status == 3) {
            qm.sendNext("몬스터를 사냥해서 전리품을 얻는 것이 아이템을 구하는 기본이자 정석인 법이지~");
            qm.dispose();
            return;
        } else {
            status--;
    ...
    switch (status) {
        ...
        case 3: {
            qm.sendAcceptDecline("상자를 열어서 #b석재#k와 #b휘장#k을 하나씩만 구해줘! 재료만 모이면 바로 멋진 의자를 만들어 줄게. 그럼 기다릴게!");
            break;
        }
        case 4: {
            qm.dispose();
...
function end(mode, type, selection) {
    ...
    switch (status) {
        ...
        case 1: {
            qm.sendNextPrev("자, 노블레스 의자야. 멋지지? #b피로할 때 의자에 앉으면 HP가 빨리 회복#k 돼. 인벤토리 #b설치창#k에 들어가 있을 테니 확인해 본 후에 #b키샤#k에게 가줘. 화살표를 따라 왼쪽으로 가면 만날 수 있어.\r\n\r\n#fUI/UIWindow.img/QuestIcon/4/0#\r\n#i3010060# 노블레스 의자 1개\r\n#fUI/UIWindow.img/QuestIcon/8/0# 95 exp");
            break;
        }
        case 2: {
            qm.dispose();
