...
function start(mode, type, selection) {
    if (mode == 1) {
        ...
        if (status == 3) {
            qm.sendNext("메이플 월드에 오신 것을 환영합니다! 여기는 메이플 월드에서도 가장 평화롭고 안전한 곳, 여제 시그너스님이 다스리는땅, 에레브입니다.");
            qm.dispose();
            return;
        } else {
            status--;
    ...
    switch (status) {
        ...
        case 3: {
            qm.sendAcceptDecline("그럼 당신께 사냥에 대해 알려드릴 #r키잔#k을 만나러 가시겠어요? 키잔은 화살표를 따라 왼쪽으로 가면 만나실 수 있답니다.");
            break;
        }
        case 4: {
            qm.dispose();
