...
function start(mode, type, selection) {
    ...
    switch (status) {
        case 0: {
            qm.sendNext("음? 이 섬에 왠 사람이지...? 어라, 리린님 아니십니까? 리린님이 여기까지 어쩐 일로... 아, 혹시 이 분은 리린님께서 아시는 분입니까? 네? 이 분이 영웅이라고요?");
            break;
        }
        case 1: {
        ...
        case 3: {
            qm.sendAcceptDecline("그런데 검은 마법사의 저주로 얼음 속에서 수백 년을 잠들어 계셔서 그런지 영웅님께서 체력이 매우 약해지신 것 같군요. #b여기 체력 회복용 포션 하나 드릴 테니 어서 드십시오.#k");
            break;
        }
        case 4: {
            qm.gainItem(2000022, 1);
            qm.forceStartQuest();
            qm.sendNextS("일단 쭈욱 들이키신 후에 다시 이야기 하죠~", 1);
            break;
        }
        case 5: {
            qm.dispose();
...
function end(mode, type, selection) {
    ...
    switch (status) {
        ...
        case 2: {
            if (qm.isQuestActive(21010) == true) {
                qm.gainItem(2000022, 5);
                qm.gainItem(2000023, 5);
                qm.forceCompleteQuest();
                qm.gainExp(16);
            }
            qm.sendNextPrevS("레벨업을 하셨군요? 혹시 스킬 포인트도 획득하셨을지 모르겠습니다. 메이플 월드에서는 레벨 1이 올라갈 때마다 스킬 포인트 3을 얻을 수 있거든요. #bK키#k를 눌러 스킬창을 열면 확인하실 수 있을 겁니다.", 1);
            break;
        }
        case 3: {
