...
function start(mode, type, selection) {
    if (mode == 1) {
        ...
        if (status == 3) {
            qm.sendNext("강해지고 싶다면 열심히 수련하는 수밖에 없어.");
            qm.dispose();
            return;
        } else {
            status--;
    ...
    switch (status) {
        ...
        case 2: {
            qm.sendAcceptDecline("확인했으면 바로 시험을 해봐야겠지? 이 주변에는 에레브에서 가장 약한 티노가 많이 있으니 한 번 사냥해 보도록 해. #r1마리#k 사냥 한 후에 돌아오면 보상을 주도록 할게.");
            break;
        }
        case 3: {
            qm.dispose();
...
function end(mode, type, selection) {
    ...
    switch (status) {
        ...
        case 1: {
            qm.sendNextPrev("노블레스용 장비야. 입고 있는 옷보다는 성능이 조금 더 좋을 거야. 보기에도 멋지고. 화살표를 따라 왼쪽으로 더 가면 동생인 #b키누#k가 있는데, 그 애를 만나기 전에 옷을 갈아입는 게 어떨까?\r\n\r\n#fUI/UIWindow.img/QuestIcon/4/0#\r\n#i1002869# 노블레스 우아한 모자 1개\r\n#i1052177# 노블레스 화려한 로브 1개\r\n#fUI/UIWindow.img/QuestIcon/8/0# 30 exp");
            break;
        }
        case 2: {
            qm.dispose();
