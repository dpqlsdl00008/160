...
function start(mode, type, selection) {
	...
	if (status == 0) {
		...
		qm.askAcceptDecline("기운 내라고! 아, 그렇지. 리린이 이번에 #b새로운 스킬#k을 해독한 것 같아. 오르비스에 있었던 일도 알려줄 겸, #b리엔으로 가서 리린을 만나봐.#k");
	} else if (status == 5) {
...
function end(mode, type, selection) {
	...
	if (status == 0) {
		...
		qm.sendOk("당장은 당신이 강해지는 게 더 중요해요. 봉인석에 관한 건 저와 진실 아저씨가 주시하고 있을 테니, 아란은 방금 드린#b콤보 스매쉬#k 스킬을 익혀 주세요.");
		qm.teachSkill(21100004, qm.getPlayer().getSkillLevel(21100004), 20);
