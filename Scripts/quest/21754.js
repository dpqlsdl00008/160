...
function start(mode, type, selection) {
	...
	if (status == 0) {
		...
		qm.askAcceptDecline("드디어 그 때 주지 못했던 편지를 드릴 수 있게 되었네요. 시간이 오래 흘러서 그런지 많이 낡았지만... 하지만 내용은 아직 읽을 수 있을 거예요.");
	} else if (status == 3) {
		...
		qm.sendOk("#b(헬레나에게 편지를 받았다...편지 안에는 어떤 내용이 있을까? 트루와 함께 살펴보자.)#k");
	} else {
		qm.dispose();
	}
}
