...
function start(mode, type, selection) {
    ...
    switch (status) {
        ...
        case 4: {
            qm.sendAcceptDecline("시험은 간단하네... 준비된 시험장에 몬스터를 물리치고 #r어둠의 힘이 담긴 구슬 30개#k를 가져오면 된다네... 그럼 바로 시험을 시작하겠나? 수락하면 시험의 숲으로 보내주지.");
            break;
        }
        case 5: {
            qm.dispose();
...
function end(mode, type, selection) {
    ...
    switch (status) {
        ...
        case 2: {
            qm.sendNextPrev("위자드는 강해져야 하네... 하지만 그 강함을 약자에게 사용하는 것은 올바른 길이 아니라네. 자신이 가지고 있는 힘을 옳은 일에 사용하는 것... 그것은 강해지는 것 이상으로 어려운 일이지...");
            break;
        }
        case 3: {
            qm.dispose();
