...
function end(mode, type, selection) {
    ...
    switch (status) {
        case 0: {
            if (qm.isQuestActive(1451) == false) {
                qm.dispose();
                qm.forceStartQuest();
            } else {
                qm.sendYesNo("그대에게 영웅의 자질이 존재함을 스스로 증명하였군. 이제 더 이상의 증명은 필요하지 않다네... 진정한 모험가가 되겠는가?");
            }
            break;
        }
        case 1: {
            if (qm.isQuestActive(1451) == true) {
                qm.gainItem(4031343, -1); 
                qm.gainItem(4031344, -1);
                qm.changeJob(qm.getJob() + 1);
                qm.forceCompleteQuest();
            }
            qm.sendNext("한 명의 자유로운 모험가에서 시작했던 그대가 지금은 강함과 선한 의지, 용기를 모두 갖춘 진정한 모험가가 되었군.");
            break;
        }
        case 2: {
