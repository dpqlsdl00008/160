...
function end(mode, type, selection) {
    ...
    switch (status) {
        case 0: {
            if (qm.isQuestActive(23013) == false) {
                qm.dispose();
                qm.forceStartQuest();
            } else {
        ...
        case 1: {
            if (qm.isQuestFinished(23013) == false) {
                qm.changeJob(3500);
        ...
        case 2: {
            qm.sendNextPrev("자칫 블랙윙에게 우리의 정체가 발각되기라도 하면 곤란합니다. 그러니 지금부터 절 담임 선생님이라고 부르십시오. 당신은 방과후 특별수업을 받으러 교실에 온 학생인 겁니다. 이 특별수업에서 당신을 강력한 메카닉으로 가르치겠습니다.");
            break;
        }
        case 3: {
            qm.dispose();
