...
public class PlayerStats implements Serializable {
    ...
    private transient int localstr, localdex, localluk, localint_, localmaxhp, localmaxmp, magic, watk, hands, accuracy, outlawhp, outlawmp;
    private transient float localmaxbasepvpdamage, localmaxbasepvpdamageL;
    private transient float localmaxbasedamage, localminbasedamage;
    public transient boolean equippedWelcomeBackRing, hasClone, Berserk;
    public transient double expBuff, dropBuff, mesoBuff, cashBuff, mesoGuard, mesoGuardMeso, expMod, pickupRange, dam_r, bossdam_r, ignoreTargetDEF;
    public transient int recoverHP, recoverMP, mpconReduce, mpconPercent, incMesoProp, reduceCooltime, coolTimeR, suddenDeathR, expLossReduceR, DAMreflect, DAMreflect_rate, ignoreDAMr, ignoreDAMr_rate, ignoreDAM, ignoreDAM_rate,
            ...
            incAllskill, combatOrders, defRange, BuffUP_Summon, evaR, speed, jump, harvestingTool,
            equipmentBonusExp, dropMod, cashMod, levelBonus, ASR, TER, pickRate, decreaseDebuff, equippedFairy, equippedSummon,
            pvpDamage, hpRecoverTime, mpRecoverTime, dot, dotTime, questBonus, pvpRank, pvpExp, wdef, mdef, trueMastery, damX, DAMreduceR;
    ...
    private transient int percent_hp, percent_mp, percent_str, percent_dex, percent_int, percent_luk, percent_acc, percent_atk, percent_matk, percent_wdef, percent_mdef,
            add_hp, add_mp, add_str, add_dex, add_int, add_luk, add_acc, add_atk, add_matk, add_wdef, add_mdef, ms_maxhp, ms_maxmp;
    private int starForce;

    public void recalcLocalStats(MapleCharacter chra) {
        recalcLocalStats(false, chra, false);
    }

    public void recalcLocalStats(MapleCharacter chra, boolean levelup) {
    ...
    public void recalcLocalStats(boolean first_login, MapleCharacter chra, boolean level) {
        if (chra.isClone()) {
        ...
        while (itera.hasNext()) {
            ...
            if (eqstat != null) {
                if (eqstat.containsKey("MHP")) {
                    //localmaxhp_ += eqstat.get("MHP");
                }
                if (eqstat.containsKey("MMP")) {
                    //localmaxmp_ += eqstat.get("MMP");
                }
                if (eqstat.containsKey("MHPr")) {
                    //percent_hp += eqstat.get("MHPr");
                }
                if (eqstat.containsKey("MMPr")) {
                    //percent_mp += eqstat.get("MMPr");
                }
            }
            ...
            if ((equip.getItemId() / 10000 == 166 && equip.getAndroid() != null || equip.getItemId() / 10000 == 167) && chra.getAndroid() == null) {
            ...
            if (equip.getState() >= 17) {
                final int[] potentials = {equip.getPotentialByLine(0), equip.getPotentialByLine(1), equip.getPotentialByLine(2)/*, equip.getPotential4(), equip.getPotential5()*/};
                for (final int i : potentials) {
                    if (i > 0) {
                        int rLevel = ii.getReqLevel(equip.getItemId()) / 10;
                        soc = ii.getPotentialInfo(i).get(rLevel > 19 ? 19 : rLevel);
                        if (soc != null) {
                ...
                for (Entry<Integer, SetItem> ent : itemz.entrySet()) {
                    if (ent.getKey() <= entry.getValue()) {
                        ...
                        if (se.option1 > 0 && se.option1Level > 0) {
                            List<StructItemOption> le = ii.getPotentialInfo(se.option1);
                            if (le == null) {
                                continue;
                            }
                            soc = le.get(se.option1Level);
                            if (soc != null) {
                                localmaxhp_ += soc.get("incMHP");
                                localmaxmp_ += soc.get("incMMP");
                                handleItemOption(soc, chra, first_login, sData);
                        ...
                        if (se.option2 > 0 && se.option2Level > 0) {
                            List<StructItemOption> le = ii.getPotentialInfo(se.option2);
                            if (le == null) {
                                continue;
                            }
                            soc = le.get(se.option2Level);
                            if (soc != null) {
                                localmaxhp_ += soc.get("incMHP");
                                localmaxmp_ += soc.get("incMMP");
                                handleItemOption(soc, chra, first_login, sData);
        ...
        Integer buff = chra.getBuffedValue(CharacterTemporaryStat.EnhancedMaxHp);
        if (buff != null) {
            localmaxhp_ += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.EnhancedMaxMp);
        if (buff != null) {
            localmaxmp_ += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.IndieMaxHp);
        if (buff != null) {
            localmaxhp_ += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.IndieMaxMp);
        if (buff != null) {
            localmaxmp_ += buff.intValue();
        }
        ...
        for (Pair<Integer, Integer> ix : chra.getCharacterCard().getCardEffects()) {
            ...
            ignoreTargetDEF += ((100 - ignoreTargetDEF) * (e.getIgnoreMob() / (double) 100));
            localstr += e.getStrX();
            localdex += e.getDexX();
            localint_ += e.getIntX();
            localluk += e.getLukX();
        ...
        if (level == true) {
            ...
            if (first_login == false) {
                ...
                statup.put(MapleStat.HP, Integer.valueOf(localmaxhp));
                statup.put(MapleStat.MP, Integer.valueOf(localmaxmp));
                chra.getClient().getSession().write(CWvsContext.OnPlayerStatChanged(statup, chra));
            }
        }
        if (GameConstants.isDemon(chra.getJob())) {
        ...
        calculateBaseDamage(Math.max(magic, watk), chra);
    ...
    private void handlePassiveSkills(final MapleCharacter chra) {
        ...
        for (Integer skillid : GameConstants.getPercentHP()) {
            if (chra.getSkillLevel(skillid) > 0) {
        ...
        for (Integer skillid : GameConstants.getPercentMP()) {
            if (chra.getSkillLevel(skillid) > 0) {
                bx = SkillFactory.getSkill(skillid);
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
        ...
        switch (chra.getJob()) {
            ...
            case 5211:
            case 5212: {
                bx = SkillFactory.getSkill(52120003);
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    BuffUP_Skill += bx.getEffect(bof).getX();
                }
                break;
            }
            case 200:
            ...
            case 1111:
            case 1112: {
                bx = SkillFactory.getSkill(11000001);
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    percent_hp += bx.getEffect(bof).getPercentHP();
                }
                    ...
                    if (chra.getDFRecoveryTimer() != null) {
                        ...
                        if (chra.getDFRecoveryTimer() == null) {
                            ...
                            TimerTask healTask = new TimerTask() {
                                ...
                                public void run() { // 데몬
                                    if (chra.isAlive()) {
                                        //chra.gainDemonSlayerForce(0, 0, DFRecovery);
                                    }
                                }
                            };
            ...
            case 312: { // Bowmaster
                ...
                bx = SkillFactory.getSkill(3120006);//스피릿링크
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    percent_hp += eff.getX();
            ...
            case 3212: {
                ...
                /* 오디너리 컨버젼 */
                bx = SkillFactory.getSkill(32110004);
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    percent_hp += bx.getEffect(bof).getPercentHP();
                }
        ...
        switch (chra.getJob()) {
            ...
            case 6512: {
                ...
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    outlawhp += bx.getEffect(bof).getMaxHpX();
                    wdef += bx.getEffect(bof).getWDEFX();
                    mdef += bx.getEffect(bof).getMDEFX();
                }
                break;
            }
            case 2002:
            ...
            case 312: {
                ...
                bx = SkillFactory.getSkill(3120010);
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    damageIncrease.put(3111003, bx.getEffect(bof).getDamage());
                    dotTime = eff.getDOTTime();
                    dot = eff.getDOT();
                }
                break;
            }
            ...
            case 122: {
                ...
                bx = SkillFactory.getSkill(1220010); //어드밴스드 차지
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    damageIncrease.put(1211002, (int) eff.getDAMRate());
                }
                break;
            }
            case 510:
    ...
    private void handleBuffStats(MapleCharacter chra) { // 여기 의심
        MapleStatEffect eff = chra.getStatForBuff(CharacterTemporaryStat.MonsterRiding);
        if (eff != null && eff.getSourceId() == 33001001) { // jaguar
            passive_sharpeye_rate += eff.getW();
            percent_hp += eff.getZ();
        }
        Integer buff = chra.getBuffedValue(CharacterTemporaryStat.Dice);
        if (buff != null) {
            percent_wdef += GameConstants.getDiceDef(buff.intValue(), 2);
            percent_mdef += GameConstants.getDiceDef(buff.intValue(), 2);
            percent_hp += GameConstants.getDiceHPMP(buff.intValue(), 3);
            percent_mp += GameConstants.getDiceHPMP(buff.intValue(), 3);
            passive_sharpeye_rate += GameConstants.getDiceCrit(buff.intValue(), 4);
            dam_r *= (GameConstants.getDiceDamage(buff.intValue(), 5) + 100.0) / 100.0;
            bossdam_r *= (GameConstants.getDiceDamage(buff.intValue(), 5) + 100.0) / 100.0;
            expBuff *= (GameConstants.getDiceExp(buff.intValue(), 6) + 100.0) / 100.0;
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Aura); // 1
        if (buff != null) {
            // dam_r *= (20 + 100.0) / 100.0;
            // bossdam_r *= (20 + 100.0) / 100.0;
            percent_wdef += eff.getZ();
            percent_mdef += eff.getZ();
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.BlueAura);
        if (eff != null) {
            percent_wdef += eff.getZ();
            percent_mdef += eff.getZ();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.ReshuffleSwitch);
        if (buff != null && buff == 1) {
            int HP_ = 10;
            if (chra.getSkillLevel(61100005) > 0) {
                HP_ += 10;
                if (chra.getSkillLevel(61110005) > 0) {
                    HP_ += 10;
                    if (chra.getSkillLevel(61120010) > 0) {
                        HP_ += 10;
                    }
                }
            }
        ...
        buff = chra.getBuffedValue(CharacterTemporaryStat.IndieMaxHpR);
        if (buff != null) {
            percent_hp += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.PMddR);
        if (buff != null) {
            percent_wdef += buff.intValue();
            percent_mdef += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.AsrR);
        if (buff != null) {
            ASR += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.TerR);
        if (buff != null) {
            TER += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Infinity);
        if (buff != null) {
            percent_matk += buff.intValue() - 1;
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.OnixGodBless);
        if (buff != null) {
            evaR += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.OnixWill);
        if (buff != null) {
            DAMreduceR += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.BeastFormMaxHP);
        if (buff != null) {
            percent_hp += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Conversion);
        if (buff != null) {
            percent_hp += buff.intValue();
        } else {
            buff = chra.getBuffedValue(CharacterTemporaryStat.MaxHp);
            if (buff != null) {
                percent_hp += buff.intValue();
            }
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.MaxMp);
        if (buff != null) {
            percent_mp += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.HowlingMaxMp);
        if (buff != null) {
            percent_mp += buff.intValue();
        }
        buff = chra.getBuffedSkill_X(CharacterTemporaryStat.MasterMagic);
        if (buff != null) {
            BuffUP_Skill += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Str);
        if (buff != null) {
            localstr += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Dex);
        if (buff != null) {
            localdex += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Int);
        if (buff != null) {
            localint_ += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Luk);
        if (buff != null) {
            localluk += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.IndieAllStat);
        if (buff != null) {
            localstr += buff.intValue();
            localdex += buff.intValue();
            localint_ += buff.intValue();
        ...
        buff = chra.getBuffedValue(CharacterTemporaryStat.EnhancedPdd);
        if (buff != null) {
            wdef += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.EnhancedMdd);
        if (buff != null) {
            mdef += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Pdd);
        if (buff != null) {
            wdef += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Mdd);
        if (buff != null) {
            mdef += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.MapleWarrior);
        if (buff != null) {
        ...
        buff = chra.getBuffedValue(CharacterTemporaryStat.EchoOfHero);
        if (buff != null) {
            final double d = buff.doubleValue() / 100.0;
            watk += (int) (watk * d);
            magic += (int) (magic * d);
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.ComboAbilityBuff);
        if (buff != null) {
            watk += buff.intValue() / 10;
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.MesoGuard);
        if (buff != null) {
            mesoGuardMeso += buff.doubleValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.ExpBuffRate);
        if (buff != null) {
            expBuff *= buff.doubleValue() / 100.0;
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.PlusExpRate);
        if (buff != null) {
            expBuff *= (1.0 + (buff.doubleValue() / 100.0));
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.CarnivalExpUp);
        if (buff != null) {
            expBuff *= (1.0 + (buff.doubleValue() / 100.0));
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.DropUpByItem);
        if (buff != null) {
            dropBuff *= buff.doubleValue() / 100.0;
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.MesoUpByItem);
        if (buff != null) {
            mesoBuff *= buff.doubleValue() / 100.0;
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.MesoUp);
        if (buff != null) {
            mesoBuff *= buff.doubleValue() / 100.0;
        }
        ...
        buff = chra.getBuffedValue(CharacterTemporaryStat.IndiePad);
        if (buff != null) {
            watk += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.IndieMad);
        if (buff != null) {
            magic += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Pad);
        if (buff != null) {
            watk += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.SpiritInfusion);
        if (buff != null) {
            passive_sharpeye_rate += buff.intValue();
            dam_r *= (buff.intValue() + 100.0) / 100.0;
            bossdam_r *= (buff.intValue() + 100.0) / 100.0;
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.EnhancedPad);
        if (buff != null) {
            watk += buff.intValue();
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.EnergyCharge);
        if (eff != null) {
            watk += eff.getWatk();
            accuracy += eff.getAcc();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Mad);
        if (buff != null) {
            magic += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Speed);
        if (buff != null) {
            speed += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.Jump);
        if (buff != null) {
            jump += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.DashSpeed);
        if (buff != null) {
            speed += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.DashJump);
        if (buff != null) {
            jump += buff.intValue();
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.PvPScoreBonus);
        if (eff != null) {
        ...
        buff = chra.getBuffedValue(CharacterTemporaryStat.HowlingAttackDamage);
        if (buff != null) {
            dam_r *= (buff.doubleValue() + 100.0) / 100.0;
            bossdam_r *= (buff.doubleValue() + 100.0) / 100.0;
        }
        buff = chra.getBuffedSkill_Y(CharacterTemporaryStat.FinalCut);
        if (buff != null) {
            dam_r *= buff.doubleValue() / 100.0;
            bossdam_r *= buff.doubleValue() / 100.0;
        }
        buff = chra.getBuffedSkill_Y(CharacterTemporaryStat.DrawBack);
        if (buff != null) {
            dam_r *= buff.doubleValue() / 100.0;
            bossdam_r *= buff.doubleValue() / 100.0;
        }
        buff = chra.getBuffedSkill_X(CharacterTemporaryStat.DojangBerserk);
        if (buff != null) {
            dam_r *= buff.doubleValue() / 100.0;
            bossdam_r *= buff.doubleValue() / 100.0;
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.Bless);
        if (eff != null) {
            watk += eff.getX();
            magic += eff.getY();
            accuracy += eff.getV();
        }
        buff = chra.getBuffedSkill_X(CharacterTemporaryStat.Concentration);
        if (buff != null) {
            mpconReduce += buff.intValue();
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.AdvancedBless);
        if (eff != null) {
            watk += eff.getX();
            magic += eff.getY();
            accuracy += eff.getV();
        ...
        eff = chra.getStatForBuff(CharacterTemporaryStat.MagicResistance);
        if (eff != null) {
            ASR += eff.getX();
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.ComboCounter);
        buff = chra.getBuffedValue(CharacterTemporaryStat.ComboCounter);
        ...
        eff = chra.getStatForBuff(CharacterTemporaryStat.Summon);
        if (eff != null) {
        ...
        eff = chra.getStatForBuff(CharacterTemporaryStat.DarkAura);
        if (eff != null) {
            dam_r *= (eff.getX() + 100.0) / 100.0;
            bossdam_r *= (eff.getX() + 100.0) / 100.0;
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.SuperBody);
        if (eff != null) {
            dam_r *= (eff.getV() + 100.0) / 100.0;
            bossdam_r *= (eff.getV() + 100.0) / 100.0;
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.Beholder);
        if (eff != null) {
            trueMastery += eff.getMastery();
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.Mechanic);
        if (eff != null) {
            passive_sharpeye_rate += eff.getCr();
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.WeaponCharge);
        if (eff != null) {
            dam_r *= eff.getDamage() / 100.0;
            //bossdam_r *= eff.getDamage() / 100.0;
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.PickPocket);
        if (eff != null) {
            pickRate = eff.getProb();
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.DamR);
        if (eff != null) {
            dam_r *= (eff.getDAMRate() + 100.0) / 100.0;
            bossdam_r *= (eff.getDAMRate() + 100.0) / 100.0;
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.AssistCharge);
        if (eff != null) {
            dam_r *= eff.getDamage() / 100.0;
            bossdam_r *= eff.getDamage() / 100.0;
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.WindWalk);
        if (eff != null) {
            dam_r *= eff.getDamage() / 100.0;
            bossdam_r *= eff.getDamage() / 100.0;
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.BlessingArmor);
        if (eff != null) {
            watk += eff.getEnhancedWatk();
        }
        buff = chra.getBuffedSkill_Y(CharacterTemporaryStat.DarkSight);
        if (buff != null) {
            dam_r *= (buff.intValue() + 100.0) / 100.0;
            bossdam_r *= (buff.intValue() + 100.0) / 100.0;
        }
        buff = chra.getBuffedSkill_X(CharacterTemporaryStat.Enrage);
        if (buff != null) {
            dam_r *= (buff.intValue() + 100.0) / 100.0;
            bossdam_r *= (buff.intValue() + 100.0) / 100.0;
        }
        buff = chra.getBuffedSkill_X(CharacterTemporaryStat.CombatOrders);
        if (buff != null) {
            combatOrders += buff.intValue();
        }
        eff = chra.getStatForBuff(CharacterTemporaryStat.SharpEyes);
        if (eff != null) {
            passive_sharpeye_rate += eff.getX();
            passive_sharpeye_percent += eff.getY();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.CriticalByItem);
        if (buff != null) {
            passive_sharpeye_rate += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.CriticalBuff);
        if (buff != null) {
            passive_sharpeye_rate += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.IgnoreTargetDEF);
        if (buff != null) {
            ignoreTargetDEF += buff.intValue();
        }
        buff = chra.getBuffedValue(CharacterTemporaryStat.HowlingCritical);
        if (buff != null) {
            passive_sharpeye_rate += buff.intValue();
        }
        if (speed > 140) {
        ...
        buff = chra.getBuffedValue(CharacterTemporaryStat.MonsterRiding);
        if (buff != null) {
            jump = 120;
    ...
    public boolean checkEquipLevels(final MapleCharacter chr, int expgain) {
        int gain = expgain;
        if (chr.isClone()) {
            return false;
        }
        ...
        for (Equip eq : all) {
            ...
            if (eq.getEquipLevel() > lvlz) { //lvlup
                for (int i = eq.getEquipLevel() - lvlz; i > 0; i--) {
                    ...
                    if (inc != null && inc.containsKey(nextLv)) {
                        eq = ii.levelUpEquip(eq, inc.get(nextLv));
                    }
                    //UGH, skillz
    ...
    public boolean checkEquipDurabilitys(final MapleCharacter chr, int gain, boolean aboveZero) {
        ...
        for (Equip item : all) {
            if (item != null && ((item.getPosition() >= 0) == aboveZero)) {
                ...
                if (item.getDurability() < 0 && item.getDurability() != -1) { //shouldnt be less than 0
        ...
        for (Equip eqq : all) {
            if (eqq != null && eqq.getDurability() == 0 && eqq.getPosition() < 0) { //> 0 went to negative
                ...
                chr.forceReAddItem(eqq, MapleInventoryType.EQUIPPED);
    ...
    private void CalcPassive_Mastery(final MapleCharacter player) {
        ...
        switch (weaponType) {
            ...
            case KATARA:
            case DAGGER:
                skil = player.getJob() >= 430 && player.getJob() <= 434 ? 4300000 : player.getSkillLevel(4220012) > 0 ? 4220012 : 4200000;
                break;
            case CROSSBOW:
            ...
            case PLANE:
                acc = false;
                skil = GameConstants.isResist(player.getJob()) ? 32100006 : (player.getJob() <= 212 ? 2100006 : (player.getJob() <= 222 ? 2200006 : (player.getJob() <= 232 ? 2300006 : (player.getJob() <= 2000 ? 12100007 : (player.getJob() <= 2712 ? 27100005 : 22120002)))));
                break;
            default:
                passive_mastery = 0;
                return;
        ...
        if (player.getJob() == 412) {
            final Skill bx = SkillFactory.getSkill(4120012);
    ...
    private void calculateFame(final MapleCharacter player) {
        ...
        for (MapleTraitType t : MapleTraitType.values()) {
            player.getTrait(t).recalcLevel(player);
        }
    }
    ...
    public final void calculateBaseDamage(final int atk, MapleCharacter chra) {
        if (atk <= 0) {
            localmaxbasedamage = 1;
        } else {
            PlayerJobType jobtype = getJobStatType(chra.getJob());
            if (jobtype == null) {
                System.out.println("[ERROR] Job type not found " + chra.getJob());
                return;
            }
            ...
            final MapleWeaponType weapon = weapon_item == null ? MapleWeaponType.NOT_A_WEAPON : InventoryConstants.getWeaponType(weapon_item.getItemId());
            final MapleWeaponType weapon2 = weapon_item2 == null ? MapleWeaponType.NOT_A_WEAPON : InventoryConstants.getWeaponType(weapon_item2.getItemId());
            if (weapon_item2 != null) {
                if (InventoryConstants.getWeaponType(weapon_item2.getItemId()) == MapleWeaponType.NOT_A_WEAPON) {
                    weapon_item2 = null;
                }
            }

            if (weapon_item == null) { // no weapon 
                localmaxbasedamage = 0;
                localminbasedamage = 0;
                return;
            }

            int mainstat = 0, secondarystat = 0;
            ...
            switch (jobtype) {
                ...
                case LUK:
                    mainstat = localluk;
                    secondarystat = GameConstants.isXenon(chra.getJob()) ? localstr : localdex;
                    break;
                case HP:
                    mainstat = hp;
                    secondarystat = localstr;
            ...
            if (weapon2 != MapleWeaponType.NOT_A_WEAPON && weapon_item != null && weapon_item2 != null) {
                ...
                localmaxbasedamage = weapon.getMaxDamageMultiplier() * (4 * mainstat + secondarystat) * ((watk - (jobtype == PlayerJobType.INT ? we2.getMatk() : we2.getWatk())) / 100.0f);
                localmaxbasedamage += weapon2.getMaxDamageMultiplier() * (4 * mainstat + secondarystat) * ((watk - (jobtype == PlayerJobType.INT ? we1.getMatk() : we1.getWatk())) / 100.0f);
            } else {
                localmaxbasedamage = weapon.getMaxDamageMultiplier() * (4 * mainstat + secondarystat) * (watk / 100.0f);
            }
            localminbasedamage = (int) (localmaxbasedamage * (trueMastery / 100.0D));;
            localminbasedamage = Math.min(localminbasedamage, localmaxbasedamage);
        }
    }
    ...
    public final void relocHeal(MapleCharacter chra) {
        ...
        if (playerjob == 111 || playerjob == 112) {
            ...
            final Skill effect = SkillFactory.getSkill(31110009);
            final int lvl = chra.getSkillLevel(effect);
            if (lvl > 0) {
    ...
    private final static int[] allJobs = {0, 10000, 10000000, 20000000, 20010000, 20020000, 20030000, 30000000, 30010000, 30020000};
    ...
    public static int getSkillByJob(final int skillID, final int job) {
        ...
        } else if (GameConstants.isDemon(job) || GameConstants.isDemonAvenger(job)) {
        ...
        } else if (GameConstants.isMukhyun(job)) {
        ...
        } else if (GameConstants.isLynn(job)) {
            return skillID + 50010000;
        } else if (GameConstants.isHayato(job)) {
            return skillID + 60000000;
        } else if (GameConstants.isAngel(job)) {
            return skillID + 60010000;
            ...
            switch (add.getLeft()) {
                ...
                case "mobcategory": //skip the category, thinkings too expensive to have yet another Map<Integer, Integer> for damage calculations
                    if (add.getMid().equals("damage")) {
                        dam_r *= (Integer.parseInt(add.getRight()) + 100.0) / 100.0;
                        bossdam_r += (Integer.parseInt(add.getRight()) + 100.0) / 100.0;
                    }
                    break;
                case "critical": // lv critical lvl?
                    ...
                    if ((job != null && canJob || job == null) && (level != null && canLevel || level == null)) {
                        switch (add.getMid()) {
                            case "prob":
                                passive_sharpeye_rate += Integer.parseInt(add.getRight());
                                break;
                            case "damage":
                                passive_sharpeye_min_percent += Integer.parseInt(add.getRight());
                                passive_sharpeye_percent += Integer.parseInt(add.getRight()); //???CONFIRM - not sure if this is max or minCritDmg
                                break;
                        }
                    }
                    break;
                ...
                case "mobdie": // lv, hpIncRatioOnMobDie, hpRatioProp, mpIncRatioOnMobDie, mpRatioProp, modify =D, don't need mob to die
                    ...
                    if ((craft == null || craft != null && chra.getTrait(MapleTraitType.craft).getLocalTotalExp() >= Integer.parseInt(craft))) {
                        switch (add.getMid()) {
                            case "hpIncOnMobDie":
                                hpRecover += Integer.parseInt(add.getRight());
                                hpRecoverProp += 5;
                                break;
                            case "mpIncOnMobDie":
                                mpRecover += Integer.parseInt(add.getRight());
                                mpRecoverProp += 5;
                                break;
                        }
                    }
                ...
                case "skill": // all these are additional skills
                    if (first_login) {
                        ...
                        if ((craft == null || craft != null && chra.getTrait(MapleTraitType.craft).getLocalTotalExp() >= Integer.parseInt(craft))) {
                            switch (add.getMid()) {
                                case "id":
                                    skillid = Integer.parseInt(add.getRight());
                                    break;
                                case "level":
                                    skilllevel = Integer.parseInt(add.getRight());
                                    break;
                            }
                        }
                    }
                ...
                case "hpmpchange":
                    switch (add.getMid()) {
                        case "hpChangerPerTime":
                            recoverHP += Integer.parseInt(add.getRight());
                            break;
                        case "mpChangerPerTime":
                            recoverMP += Integer.parseInt(add.getRight());
                            break;
                    }
                    break;
                case "statinc":
                    ...
                    switch (add.getMid()) {
                        case "incPAD":
                            watk += Integer.parseInt(add.getRight());
                            break;
                        case "incMAD":
                            magic += Integer.parseInt(add.getRight());
                            break;
                        case "incSTR":
                            localstr += Integer.parseInt(add.getRight());
                            break;
                        case "incDEX":
                            localdex += Integer.parseInt(add.getRight());
                            break;
                        case "incINT":
                            localint_ += Integer.parseInt(add.getRight());
                            break;
                        case "incLUK":
                            localluk += Integer.parseInt(add.getRight());
                            break;
                        case "incJump":
                            jump += Integer.parseInt(add.getRight());
                            break;
                        case "incMHP":
                            localmaxhp_x += Integer.parseInt(add.getRight());
                            break;
                        case "incMMP":
                            localmaxmp_x += Integer.parseInt(add.getRight());
                            break;
                        case "incPDD":
                            wdef += Integer.parseInt(add.getRight());
                            break;
                        case "incMDD":
                            mdef += Integer.parseInt(add.getRight());
                            break;
                        case "incACC":
                            accuracy += Integer.parseInt(add.getRight());
                            break;
                        case "incEVA":
                            break;
                        case "incSpeed":
                            speed += Integer.parseInt(add.getRight());
                            break;
                        case "incMMPr":
                            percent_mp += Integer.parseInt(add.getRight());
                            break;
                    }
                    break;
            }
    ...
    public final boolean setHp(int newhp, boolean silent, MapleCharacter chra) {
        ...
        if (chra != null) {
            ...
            if (oldHp > hp && !chra.isAlive()) {
                chra.getClient().removeClickedNPC();
                NPCScriptManager.getInstance().dispose(chra.getClient());
                chra.setConversation(0);
                if (chra.getBuffedValue(CharacterTemporaryStat.ReviveOnce) != null) {
                    this.hp = localmaxhp;
                    this.mp = localmaxmp;
                    chra.updateSingleStat(MapleStat.HP, this.hp);
                    chra.updateSingleStat(MapleStat.MP, this.mp);
                    chra.getClient().getSession().write(EffectPacket.showOwnBuffEffect(24111002, 7, chra.getLevel(), 1, (byte) 1));
                    chra.cancelEffectFromBuffStat(CharacterTemporaryStat.ReviveOnce);
                } else {
                    chra.playerDead();
                }
            }
            if (GameConstants.isDemonAvenger(chra.getJob())) {
                chra.getClient().getSession().write(BuffPacket.giveDemonWatk(hp));
            }
        }
        return hp != oldHp;
    }
    ...
    public final double getExpBuff() {
        return expBuff * expMod;
    }

    public final double getMesoBuff() {
        return mesoBuff + incMesoProp;
    }

    public final double getDropBuff() {
        return dropBuff * dropMod;
    }
    ...
    public final int getOutlawMaxHp() {
        return outlawhp;
    }

    public final int getOutlawMaxMp() {
        return outlawmp;
    }
    ...
    public final boolean isRangedJob(final int job) {
        if (GameConstants.isMercedes(job) || GameConstants.isCannon(job) || job == 400 || (job / 10 == 52) || (job / 100 == 3) || (job / 100 == 13) || (job / 100 == 14) || (job / 100 == 33) || (job / 100 == 35) || (job / 10 == 41)) {
            return true;
        }
        return false;
    }
