...
public class SpeedQuiz {
    ...
    private final static int INITIAL_QUESTION = 50; // Default 50 questions.
    private final static int TIME = 15; // Default 15 seconds
    ...
    private void getNewQuestion(final MapleClient c, final int questionNo) {
        ...
        c.getSession().write(CField.NPCPacket.getSpeedQuiz(npc, random.getType(), random.getObjectId(), points, questionNo, TIME));
    ...
    private void CheckAnswer(final String answerGiven) {
        if (answerGiven == null || answerGiven.equals("")) {
            return;
        } else if (answer.equalsIgnoreCase(answerGiven)) { // Loosen a bit i guess...let them any case also can. official must be exact
            points++;
        }
    }
    ...
    private void giveUp(final MapleClient c) {
        final NPCConversationManager cm = NPCScriptManager.getInstance().getCM(c);
        if (points > 0) {
            c.getPlayer().gainMeso(100 * points, true, true); // todo change reward? give up = lesser rewards
        }
    ...
    private void reward(final MapleClient c) {
        ...
        if (points == 50) {
            if (cm.canHold(1302000, 1)) {
                ...
                c.getPlayer().gainMeso(100000000, true, true);
            } else {
            ...
            c.getPlayer().gainMeso(100 * points, true, true);
        }
        cm.dispose();
        c.getPlayer().setSpeedQuiz(null);
    }
