...
public class InternCommand {
    ...
    public static class WhereAmI extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            c.getPlayer().dropMessage(5, "You are on map " + c.getPlayer().getMap().getId()
                    + " Position: " + c.getPlayer().getPosition()
                    + " Foothold: " + c.getPlayer().getFH());
            return 1;
        }
    }
    ...
    public static class Test extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            //c.getSession().write(CField.UIPacket.startAzwan());
            return 1;
        }
    }
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            try {
                if (splitted[1].equals("전부")) {
                    ...
                    c.getPlayer().dropMessage(6, "Clear inventory syntax: !인벤토리초기화 <전부, 장착장비, 장비, 소비, 설치, 기타, 캐시>");
                    c.getPlayer().dropMessage(6, "WARNING: THIS WILL WIPE THE TAB SELECTED. THIS CAN NOT BE UNDONE.");
                }
    ...
    public static class 전체동접2 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            for (int i = 1; i <= channels; i++) {
                c.getPlayer().dropMessage(6, "Channel " + i + ": " + ChannelServer.getInstance(i).getPlayerStorage().getOnlinePlayers(true));
            }
            return 1;
        }
    }
    ...
    public static class GM extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            String message = sender + "[CH" + c.getChannel() + "]: " + StringUtil.joinStringFrom(splitted, 1);
            for (ChannelServer ch : ChannelServer.getAllInstances()) {
                for (MapleCharacter chr : ch.getPlayerStorage().getAllCharacters()) {
                    if (chr.getGMLevel() >= 2) {
    ...
    public static class Reports extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (cheaterSize <= 0) {
                c.getPlayer().dropMessage(6, "No current reports.");
            }
    ...
    public static class RR extends CommandExecute {
    ...
    public static class 좌표 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            c.getPlayer().dropMessage(6, "MAP : " + c.getPlayer().getMapId() + " (" + c.getPlayer().getMap().getMapName() + ") | X: " + pos.x + " | Y: " + pos.y + " | RX0: " + (pos.x + 50) + " | RX1: " + (pos.x - 50) + " | FH: " + c.getPlayer().getFH());
            return 1;
        }
    }
