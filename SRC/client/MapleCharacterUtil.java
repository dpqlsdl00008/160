...
public class MapleCharacterUtil {
    ...
    public static final boolean canCreateChar(String name, boolean gm){
        ...
        if (name.getBytes("EUC-KR").length < 4 || name.getBytes("EUC-KR").length > 13 || getIdByName(name) != -1) {
            return false;
        }
        //if (name.contains("타락파워전사") || name.contains("리첸시아") || name.contains("디브릿지")) {
            //return false;
        //}
        } catch(Exception e) {
            //e.printStackTrace();
    ...
    public static final boolean isEligibleCharName(String name, boolean gm) {
        if (name.getBytes().length < 2 || name.getBytes().length > 14) {
            return false;
        }
        if (gm) {
            return true;
    ...
    public static final boolean canChangePetName(final String name) {
        if (name.getBytes(Charset.forName("MS949")).length > 13) {
            return false;
        }
    ...
    public static final String getNameById(final int id) {
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
    ...
    public static void setNXCodeUsed(String name, String code) {
        ...
        try {
            ...
            Logger.getLogger(MapleCharacterUtil.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            try {
                if (ps != null) {
                    ps.close();
