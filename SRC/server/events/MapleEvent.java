...
public abstract class MapleEvent {
    ...
    public void BigOut(MapleCharacter chr) {
        final MapleMap mapp = chr.getClient().getChannelServer().getMapFactory().getMap(910000000);
        chr.changeMap(mapp, mapp.getPortal(0));
    }
    
    public void BigIn(MapleCharacter chr) {
        final MapleMap mapp = chr.getClient().getChannelServer().getMapFactory().getMap(910023100);
        chr.changeMap(mapp, mapp.getPortal(0));
    }
...
//        }
            ...
            for (MapleEventType t : MapleEventType.values()) {
                ...
                if (e.isRunning) {
                    for (int i : e.type.mapids) {
                        if (cserv.getEvent() == i) {
			    World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "이벤트 참가 모집이 종료되었습니다."));
                            e.broadcast(CWvsContext.serverNotice(0, "이벤트가 30초 후 시작됩니다!"));
                            e.broadcast(CField.getClock(30));
                            cserv.eventMapwarp = 0; // 이벤트 맵 입장 코드 초기화
    ...
    public static final String scheduleEvent(final MapleEventType event, final ChannelServer cserv) {
        ...
        for (int i : cserv.getEvent(event).type.mapids) {
            if (cserv.getMapFactory().getMap(i).getCharactersSize() > 0) {
                return "이벤트 맵에 누군가 있습니다.";
            }
        }
        cserv.setEvent(cserv.getEvent(event).type.mapids[0]);
        cserv.getEvent(event).reset(); // ㅎㅁㅁ...
        cserv.eventMapwarp = cserv.getEvent(event).type.mapids[0];
        World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "현재 < " + StringUtil.makeEnumHumanReadable(event.name()) + " > 이벤트가 " + cserv.getChannel() + "채널에서 시작됩니다! 채널을 변경하여 " + cserv.getChannel() + "채널 광장에서 입장하세요!"));
        return "";
    }
    ...
    public static final String cancelEvent(final MapleEventType event, final ChannelServer cserv) {
        if (cserv.getEvent() != -1 || cserv.getEvent(event) == null) {
            ...
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "Hello " + LoginServer.getServerName() + "! The " + StringUtil.makeEnumHumanReadable(event.name()) + " event in channel " + cserv.getChannel() + " has been cancelled. Sorry for the inconvenience."));
        } else {
