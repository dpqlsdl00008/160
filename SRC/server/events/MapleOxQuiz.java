...
public class MapleOxQuiz extends MapleEvent {
    ...
    public void sendQuestion(final MapleMap toSend) {
        ...
        if (timesAsked == 6) {
            toSend.broadcastMessage(CWvsContext.serverNotice(0, "축하합니다~ 준비된 퀴즈를 모두 맞추셨습니다! 잠시 후 이벤트 상품을 지급받을 수 있습니다."));
        } else {
            getMap(0).broadcastMessage(CWvsContext.serverNotice(0, "20초 후 문제가 출제됩니다. 모두 준비해 주세요!"));
        }
        getMap(0).broadcastMessage(CField.getClock(20));
        oxSchedule2 = EventTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                ...
                if (timesAsked == 10) { // 이건가? toSend.getCharactersSize() - number <= 1 
                    toSend.broadcastMessage(CWvsContext.serverNotice(6, "이벤트가 끝났습니다!"));
                    unreset();
...
//                            chr.finishAchievement(19);
           ...
           //     toSend.broadcastMessage(CField.showOXQuiz(question.getKey().left, question.getKey().right, true));
            //    toSend.broadcastMessage(CField.getClock(10)); //quickly change to 12
                toSend.broadcastMessage(CField.stopClock());
                toSend.broadcastMessage(CField.showOXQuiz(question.getKey().left, question.getKey().right, true));
            }
        }, 20000);
        if (oxSchedule != null) {
