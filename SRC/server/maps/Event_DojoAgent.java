...
public class Event_DojoAgent {
    ...
    public static boolean warpNextMap(final MapleCharacter c, final boolean fromResting, final MapleMap currentmap) {
        try {
            ...
            if (currentmap.getId() == 925064000) {
                MapleMap exitMap = ch.getMapFactory().getMap(925020002);
                c.changeMap(exitMap, exitMap.getPortal(0));
                return true;
            }
    ...
    private static final void spawnMonster(final MapleMap map, final int stage) {
        ...
        if (mobid != 0) {
            ...
            MapTimer.getInstance().schedule(new Runnable() {
                ...
                @Override
                public void run() {
                    map.spawnMonsterWithEffect(MapleLifeFactory.getMonster(mobid), 15, rand == 0 ? point1 : rand == 1 ? point2 : point3, 0);
                }
            }, 3000);
        }
    }
