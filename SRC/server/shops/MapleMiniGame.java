...
public class MapleMiniGame extends AbstractPlayerStore {
    ...
    public void checkWin() {
        if (getDPoints() >= getMatchesToWin() && !isOpen()) {
            ...
            this.broadcastToVisitors(CPlayerShop.getMiniGameResult(this, tie ? 1 : 2, x));
            this.setOpen(true);
            update();
            checkExitAfterGame();
        }
    ...
    public void send(MapleClient c) {
        ...
        c.getSession().write(CPlayerShop.getMiniGame(c, this));
    }
    ...
    public void setPiece(int move1, int move2, int type, MapleCharacter chr) {
        if (piece[move1][move2] == 0 && !isOpen()) {
            ...
            //this.broadcastToVisitors(PlayerShopPacket.getMiniGameMoveOmok1(move1, move2, type)); 이곳에 놓을수 없습니다(3,3 체크해주는구간)
            this.broadcastToVisitors(CPlayerShop.getMiniGameMoveOmok(move1, move2, type));
            boolean found = false;
            for (int y = 0; y < 15; y++) {
                for (int x = 0; x < 15; x++) {
                    if (!found && searchCombo(x, y, type)) {
                        this.broadcastToVisitors(CPlayerShop.getMiniGameResult(this, 2, getVisitorSlot(chr)));
                        this.setOpen(true);
                        update();
                        checkExitAfterGame();
                        found = true;
