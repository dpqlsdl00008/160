...
public class HiredMerchant extends AbstractPlayerStore {
    ...
    public void buy(MapleClient c, int item, short quantity) {
        ...
        if (MapleInventoryManipulator.checkSpace(c, newItem.getItemId(), newItem.getQuantity(), newItem.getOwner())) {
            ...
            if (gainmeso > 0) {
                ...
                //MapleInventoryManipulator.removeById(c.getPlayer().getClient(), GameConstants.getInventoryType(4310114), 4310114, theQuantity, true, false);
                //c.getPlayer().getClient().sendPacket(CWvsContext.InfoPacket.getShowItemGain(4310114, -theQuantity, true));
    ...
    public void closeShop(boolean saveItems, boolean remove) {
        ...
        if (remove) {
            ...
            getMap().broadcastMessage(CPlayerShop.destroyHiredMerchant(getOwnerId()));
        }
    ...
    public void sendDestroyData(MapleClient client) {
        if (isAvailable()) {
            client.getSession().write(CPlayerShop.destroyHiredMerchant(getOwnerId()));
        }
    }
    ...
    public void sendSpawnData(MapleClient client) {
        if (isAvailable()) {
            client.getSession().write(CPlayerShop.spawnHiredMerchant(this));
        }
    }
    ...
    public final void sendBlackList(final MapleClient c) {
        c.getSession().write(CPlayerShop.MerchantView((short) 0x25, blacklist));
    }

    public final void sendVisitor(final MapleClient c) {
        c.getSession().write(CPlayerShop.MerchantView((short) 0x24, visitors));
    }
}
