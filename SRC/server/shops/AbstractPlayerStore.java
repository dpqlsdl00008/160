...
public abstract class AbstractPlayerStore extends MapleMapObject implements IMaplePlayerShop {
    ...
    public boolean saveItems() {
        ...
        try {
            ...
            ps = con.prepareStatement("INSERT INTO hiredmerch (characterid, accountid, Mesos, time, map, x) VALUES (?, ?, ?, ?, ?, ?)", DatabaseConnection.RETURN_GENERATED_KEYS);
            ps.setInt(1, ownerId);
            ps.setInt(2, owneraccount);
            ps.setInt(3, meso.get());
            ps.setLong(4, System.currentTimeMillis());
            ps.setInt(5, user.getMapId());
            ps.setInt(6, user.getClient().getChannel());
            ps.executeUpdate();
    ...
    public void update() {
        if (isAvailable()) {
            if (getShopType() == IMaplePlayerShop.HIRED_MERCHANT) {
                getMap().broadcastMessage(CPlayerShop.updateHiredMerchant((HiredMerchant) this));
            } else if (getMCOwner() != null) {
                getMap().broadcastMessage(CPlayerShop.sendPlayerShopBox(getMCOwner()));
            }
        }
    }
    ...
    public void addVisitor(MapleCharacter visitor) {
        ...
        if (i > 0) {
            if (getShopType() >= 3) {
                broadcastToVisitors(CPlayerShop.getMiniGameNewVisitor(visitor, i, (MapleMiniGame) this));
            } else {
                broadcastToVisitors(CPlayerShop.shopVisitorAdd(visitor, i));
            }
    ...
    public void removeVisitor(MapleCharacter visitor) {
        ...
        if (slot > 0) {
            broadcastToVisitors(CPlayerShop.shopVisitorLeave(slot), slot);
            visitor.getClient().getSession().write(CPlayerShop.shopErrorMessage(4, 1)); // 방에서 퇴장했습니다.
    ...
    public void removeAllVisitors(int error, int type) {
        for (int i = 0; i < chrs.length; i++) {
            ...
            if (visitor != null) {
                if (type != -1) {
                    visitor.getClient().getSession().write(CPlayerShop.shopErrorMessage(error, type));
                }
                broadcastToVisitors(CPlayerShop.shopVisitorLeave(getVisitorSlot(visitor)), getVisitorSlot(visitor));
