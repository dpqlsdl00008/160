...
public class MapleAchievement {
    ...
    public void finishAchievement(MapleCharacter chr) {
        ...
        if (notice && !chr.isGM()) {
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "[Achievement] Congratulations to " + chr.getName() + " on " + name + " and rewarded with " + reward / 2 + " cash!"));
        } else {
            chr.getClient().getSession().write(CWvsContext.serverNotice(5, "[Achievement] You've gained " + reward / 2 + " Cash as you " + name + "."));
        }
    }
}
