...
public class MapleTrade {
    ...
    public final void chat(final String message) throws Exception {
        ...
        if (partner != null) {
            partner.getChr().getClient().getSession().write(CPlayerShop.shopChat(chr.get().getName() + " : " + message, 1));
        }
    ...
    public final void chatAuto(final String message) {
        ...
        if (partner != null) {
            partner.getChr().getClient().getSession().write(CPlayerShop.shopChat(message, 1));
        }
        if (chr.get().getClient().isMonitored()) { //Broadcast info even if it was a command.
    ...
    public final boolean setItems(final MapleClient c, final Item item, byte targetSlot, final int quantity) {
        ...
        if (item.getItemId() == 2430117 || item.getItemId() == 2430118) {
    ...
    public static final void startCashTrade(final MapleCharacter c) {
        if (c.getTrade() == null) {
            c.setTrade(new MapleTrade((byte) 0, c));
            c.getClient().getSession().write(InteractionPacket.getTradeCashStart(c.getClient(), c.getTrade(), (byte) 0));
        } else {
            c.getClient().getSession().write(CWvsContext.serverNotice(5, "You are already in a trade"));
        }
    }
    ...
    public static final void inviteCashTrade(final MapleCharacter c1, final MapleCharacter c2) {
        ...
        if (c2 != null && c2.getTrade() == null) {
            ...
            c2.getTrade().setPartner(c1.getTrade());
            c1.getTrade().setPartner(c2.getTrade());
            c2.getClient().getSession().write(InteractionPacket.getTradecCashInvite(c1));
        } else {
            c1.getClient().getSession().write(CWvsContext.serverNotice(5, "The other player is already trading with someone else."));
            cancelTrade(c1.getTrade(), c1.getClient(), c1);
        }
    ...
    public static final void visitTrade(final MapleCharacter c1, final MapleCharacter c2) {
        if (c2 != null && c1.getTrade() != null && c1.getTrade().getPartner() == c2.getTrade() && c2.getTrade() != null && c2.getTrade().getPartner() == c1.getTrade()) {
            ...
            c2.getClient().getSession().write(CPlayerShop.shopVisitorAdd(c1, 1));
            c1.getClient().getSession().write(InteractionPacket.getTradeStart(c1.getClient(), c1.getTrade(), (byte) 1));
        } else {
            c1.getClient().getSession().write(CWvsContext.serverNotice(5, "이미 다른요청을 처리중입니다."));
        }
    }
    ...
    public static final void visitCashTrade(final MapleCharacter c1, final MapleCharacter c2) {
        if (c2 != null && c1.getTrade() != null && c1.getTrade().getPartner() == c2.getTrade() && c2.getTrade() != null && c2.getTrade().getPartner() == c1.getTrade()) {
            ...
            c1.getTrade().inTrade = true;
            c2.getClient().getSession().write(CPlayerShop.shopVisitorAdd(c1, 1));
            c1.getClient().getSession().write(InteractionPacket.getTradeCashStart(c1.getClient(), c1.getTrade(), (byte) 1));
        } else {
            c1.getClient().getSession().write(CWvsContext.serverNotice(5, "이미 다른요청을 처리중입니다."));
        }
    }
