...
public class MapleQuestAction implements Serializable {
    ...
    private static boolean canGetItem(QuestItem item, MapleCharacter c) {
        ...
        if (item.job > 0) {
            ...
            boolean jobFound = true; // false // 임시
            for (int codec : code) {
    ...
    public void runStart(MapleCharacter c, Integer extSelection) {
        ...
        switch (type) {
            case exp:
                ...
                if (c.getLevel() < 10) {
                    c.gainExp((intStore * 1), true, true, true);
                } else {
                    c.gainExp((intStore * GameConstants.getExpRate_Quest() * ((c.getTrait(MapleTraitType.sense).getLevel() * 3 / 10) + 100) / 100), true, true, true);
                }
                break;
            case item:
            ...
            case nextQuest:
                ...
                c.getClient().sendPacket(CUserLocal.questResult(QuestType.QuestRes_Act_Success, quest.getId(), status.getNpc(), intStore));
                break;
            case money:
                status = c.getQuest(quest);
                if (status.getForfeited() > 0) {
...
//		MapleQuest.getInstance(intStore).forceComplete(c, 0);
        ...
        switch (type) {
                ...
                for (QuestItem item : items) {
                    ...
                    if (count < 0) { // remove items
                        if (!c.haveItem(id, count, false, true)) {
                            c.dropMessage(1, "아이템을 받을 수 없습니다.");
                            return false;
                        }
                    } else { // add items
                        if (MapleItemInformationProvider.getInstance().isPickupRestricted(id) && c.haveItem(id, 1, true, false)) {
                            c.dropMessage(1, MapleItemInformationProvider.getInstance().getName(id) + " 해당 아이템은 1개만 소지 가능합니다.");
                            return false;
                        }
            ...
            case money: {
                ...
                if (c.getMeso() + meso < 0) { // Giving, overflow
                    c.dropMessage(1, "메소의 최대치가 넘었습니다.");
                    return false;
    ...
    public void runEnd(MapleCharacter c, Integer extSelection) { // 버섯의성 아이템 차감은 내일~~
        ...
        switch (type) {
            case exp: {
                ...
                if (c.getLevel() < 10) {
                    ...
                    c.gainExp((intStore * GameConstants.getExpRate_Quest() * (c.getStat().questBonus) * ((c.getTrait(MapleTraitType.sense).getLevel() * 3 / 10) + 100) / 100), true, true, true);
                }
                break;
            }
            /*
            ...
            */
            case item: {
                ...
                for (QuestItem item : items) {
                    ...
                    if (count < 1) {
                        ...
                        c.getClient().getSession().write(InfoPacket.getShowItemGain(id, realcount, true));
                    } else {
                        final int period = item.period / 1440;
                        final String name = MapleItemInformationProvider.getInstance().getName(id);
            ...
            case nextQuest: {
                c.getClient().sendPacket(CUserLocal.questResult(QuestType.QuestRes_Act_Success, quest.getId(), c.getQuest(quest).getNpc(), intStore));
                break;
            }
            case money: {
                c.gainMeso(intStore, true, true);
...
//		MapleQuest.getInstance(intStore).forceComplete(c, 0);
    ...
    private static List<Integer> getJobBy5ByteEncoding(int encoded) {
        ...
        /*
        System.out.println("(encoded & 0x1) : " + (encoded & 0x1));
        System.out.println("(encoded & 0x2) : " + (encoded & 0x2));
        System.out.println("(encoded & 0x4) : " + (encoded & 0x4));
        System.out.println("(encoded & 0x8) : " + (encoded & 0x8));
        System.out.println("(encoded & 0x10) : " + (encoded & 0x10));
        System.out.println("(encoded & 0x20) : " + (encoded & 0x20));
        System.out.println("(encoded & 0x40) : " + (encoded & 0x40));
        System.out.println("(encoded & 0x80) : " + (encoded & 0x80));
        System.out.println("(encoded & 0x100) : " + (encoded & 0x100));
        System.out.println("(encoded & 0x200) : " + (encoded & 0x200));
        System.out.println("(encoded & 0x400) : " + (encoded & 0x400));
        System.out.println("(encoded & 0x800) : " + (encoded & 0x800));
        System.out.println("(encoded & 0x1000) : " + (encoded & 0x1000));
        System.out.println("(encoded & 0x2000) : " + (encoded & 0x2000));
        System.out.println("(encoded & 0x4000) : " + (encoded & 0x4000));
        System.out.println("(encoded & 0x8000) : " + (encoded & 0x8000));
        System.out.println("(encoded & 0x10000) : " + (encoded & 0x10000));
        System.out.println("(encoded & 0x20000) : " + (encoded & 0x20000));
        System.out.println("(encoded & 0x40000) : " + (encoded & 0x40000));
        System.out.println("(encoded & 0x80000) : " + (encoded & 0x80000));
        System.out.println("(encoded & 0x100000) : " + (encoded & 0x100000));
        System.out.println("(encoded & 0x200000) : " + (encoded & 0x200000));
        System.out.println("(encoded & 0x400000) : " + (encoded & 0x400000));
        System.out.println("(encoded & 0x800000) : " + (encoded & 0x800000));
        System.out.println("(encoded & 0x1000000) : " + (encoded & 0x1000000));
        System.out.println("(encoded & 0x2000000) : " + (encoded & 0x2000000));
        System.out.println("(encoded & 0x4000000) : " + (encoded & 0x4000000));
        System.out.println("(encoded & 0x8000000) : " + (encoded & 0x8000000));
        System.out.println("(encoded & 0x10000000) : " + (encoded & 0x10000000));
        System.out.println("(encoded & 0x20000000) : " + (encoded & 0x20000000));
        System.out.println("(encoded & 0x40000000) : " + (encoded & 0x40000000));
        System.out.println("(encoded & 0x80000000) : " + (encoded & 0x80000000));
         */
