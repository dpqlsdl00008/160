...
public class PlayerNPC extends MapleNPC implements MapleCharacterLook {
    ...
    public PlayerNPC(MapleCharacter chr, String name, int npc, MapleMap map, int x, int y, int f, int fh) {
        super(npc, name);
    ...
    public void update(MapleCharacter chr) {
        ...
        setHair(chr.getHair());
        setFace(chr.getFace());
        setSkin((byte) (chr.getSkin()));
        setGender(chr.getGender());
        setPets(chr.getPets());

        equips = new HashMap<Short, Integer>();
    ...
    public void sendSpawnData(MapleClient client) {
        client.getSession().write(NPCPacket.spawnNPC(this, true, -1));
                ...
                for (ChannelServer cserv : ChannelServer.getAllInstances()) {
                    for (PlayerNPC npc : cserv.getAllPlayerNPC()) {
                        if (npc.getId() == npcid) {
                            if (update) {
                                npc.update(chr);
                                map.broadcastMessage(NPCPacket.removeNPC(npc.getObjectId()));
                                map.broadcastMessage(NPCPacket.removeNPCController(npc.getObjectId()));
                            }
                            map.broadcastMessage(NPCPacket.spawnNPC((PlayerNPC) npc, true, -1));
                            map.broadcastMessage(CWvsContext.spawnPlayerNPC((PlayerNPC) npc));
                            map.broadcastMessage(NPCPacket.spawnNPCRequestController((PlayerNPC) npc, true));
                        }
                    }
                }
            }
