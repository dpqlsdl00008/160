...
public class SpawnPoint extends Spawns {
	...
	for (MapleSummon s : map.getAllSummonsThreadsafe()) {
		...
		final MapleStatEffect effect = SkillFactory.getSkill(s.getSkill()).getEffect(s.getSkillLevel());
                for (Map.Entry<MonsterTemporaryStat, Integer> stat : effect.getMonsterStati().entrySet()) {
                    mob.applyStatus(s.getOwner(), new MonsterTemporaryStatEffect(stat.getKey(), stat.getValue(), s.getSkill(), null, false), false, effect.getDuration(), true,effect);
                }
		break;
	    }
	}
