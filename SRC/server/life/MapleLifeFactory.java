...
public class MapleLifeFactory {
    ...
    private static Map<Integer, String> npcFunc = new HashMap<Integer, String>();
        ...
        for (MapleData c : npcStringData) {
            ...
            try {
                if (npcData.getData(n) != null) {//only thing we really have to do is check if it exists. if we wanted to, we could get the script as well :3
                    ...
                    String func = MapleDataTool.getString("func", c, "noFunc");
                    npcFunc.put(nid, func);
                }
    ...
    public static MapleMonsterStats getMonsterStats(int mid) {
        ...
        if (stats == null) {
            ...
            stats.setHp((!stats.isBoss() && MapleDataTool.getIntConvert("maxHP", monsterInfoData) != 1) ? (MapleDataTool.getIntConvert("maxHP", monsterInfoData) / 2) : MapleDataTool.getIntConvert("maxHP", monsterInfoData));
            ...
            if (buffme && stats.getLevel() > 100) {
                stats.setPhysicalAttack((int) (MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0)));
                stats.setMagicAttack((int) (MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0)));
            } else {
            ...
            stats.setSummonEffect((byte) MapleDataTool.getIntConvert("summonEffect", monsterInfoData, 0));
            ...
            stats.setHideName(MapleDataTool.getIntConvert("hideName", monsterInfoData, 0));
            ...
            if (monsterInfoData.getChildByPath("finalmaxHP") != null) {
                stats.setfinalmaxHP(Long.parseLong(MapleDataTool.getString("finalmaxHP", monsterInfoData)));
            }
            ...
            final MapleData patrolData = monsterInfoData.getChildByPath("patrol");
            if (patrolData != null) {
                stats.setPatrolMob(true);
                stats.setRange(MapleDataTool.getIntConvert("range", patrolData, 0));
                stats.setDetectX(MapleDataTool.getIntConvert("detectX", patrolData, 0));
                stats.setSenseX(MapleDataTool.getIntConvert("senseX", patrolData, 0));
            }
            ...
            if (monsterSkillData != null) {
                ...
                while (monsterSkillData.getChildByPath(Integer.toString(i)) != null) {
                    MobSkill ms = MobSkillFactory.getMobSkill(Integer.valueOf(MapleDataTool.getInt(i + "/skill", monsterSkillData, 0)).intValue(), Integer.valueOf(MapleDataTool.getInt(i + "/level", monsterSkillData, 0)).intValue());
                    if (ms != null) {
                        int skillAfter = Integer.valueOf(MapleDataTool.getInt(i + "/skillAfter", monsterSkillData, 0)).intValue();
            ...
            for (int i = 0; true; i++) { // TODO: Check and reprint all available values again..doing like below is a ridiculous way
                ...
                boolean notMissAttack = monsterAtt.getChildByPath("notMissAttack") != null;
                if (!notMissAttack) {
                    notMissAttack = attackData.getChildByPath("notMissAttack") != null;
                }
                ret.setNotMissAttack(notMissAttack);
                ...
                int fixDamage = MapleDataTool.getInt("fixDamage", monsterAtt, 0);
                if (fixDamage == 0) {
                    fixDamage = MapleDataTool.getInt("fixDamage", attackData, 0);
                }
                ret.setFixDamage(fixDamage);
                
                int attackCount = MapleDataTool.getInt("attackCount", monsterAtt, 0);
                if (attackCount == 0) {
                    attackCount = MapleDataTool.getInt("attackCount", attackData, 0);
                }
                ret.setAttackCount(attackCount);
    ...
    public static MapleNPC getNPC(final int nid) {
        ...
        String func = npcFunc.get(nid);
        if (func == null) {
            func = "";
        }
        return new MapleNPC(nid, name, func);
    }
    ...
    public final List<Pair<Integer, String>> getAllStringData() {
        final List<Pair<Integer, String>> monsterPairs = new ArrayList<Pair<Integer, String>>();
        MapleData monstersData;
        monstersData = stringDataWZ.getData("Mob.img");
        for (final MapleData monsterFolder : monstersData.getChildren()) {
            monsterPairs.add(new Pair<Integer, String>(Integer.parseInt(monsterFolder.getName()), MapleDataTool.getString("name", monsterFolder, "NO-NAME")));
        }
        return monsterPairs;
    }
}
