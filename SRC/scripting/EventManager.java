...
public class EventManager {
    ...
    public void startInstance_Expedition(String mapid, MapleCharacter user) {
        try {
            EventInstanceManager eim = (EventInstanceManager) iv.invokeFunction("setup", (Object) mapid);
            eim.registerExpedition(World.Party.getExped(user.getParty().getExpeditionId()), user.getMap());
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    public void startInstance(final MapleParty party, final MapleMap map, final int maxLevel, int time) {
        Timer.CloneTimer tMan = Timer.CloneTimer.getInstance();
        Runnable r = new Runnable() {
            @Override
            public void run() {
                startInstance(party, map, maxLevel);
            }
        };
    ...
    public void startInstance(MapleSquad squad, MapleMap map, int questID) {
        ...
        if (!squad.getLeader().isGM()) {
            //hackfix -- remove this later
            /*if (!name.equals("CygnusBattle") && !name.equals("HorntailBattle") && squad.getMembers().size() < squad.getType().i) { //less than 3
    ...
    public void warpAllPlayer(int from, int to) {
        ...
        if (frommap == null) {
            //System.out.println("warpAllPlayer frommap (" + from + ") is null.");
        } else if (tomap == null) {
            //System.out.println("warpAllPlayer tomap (" + to + ") is null.");
        } else {
    ...
    public void broadcastShip(final int mapid, final int mode) {
        broadcastShip(mapid, 0, mode);
    }
        ...
        if (!weather) {
            ...
            for (MapleMap load : getMapFactory().getAllMaps()) {
                if (load.getCharactersSize() > 0) {
                    load.floatNotice(msg, type, false);
                }
            }
        }
    }
    ...
    public boolean scheduleRandomEventInChannel(int chz) {
        ...
        while (t == null) {
            for (MapleEventType x : MapleEventType.values()) {
                if (Randomizer.nextInt(MapleEventType.values().length) == 0 && x != MapleEventType.퀴즈) {
                    t = x;
                    break;
                }
            }
    ...
    public int getMonsterCount(int mapid, int mobid) {
        return getMapFactory().getMap(mapid).countMonsterById(mobid);
    }
    ...
    public void broadcastTopMsg(final String msg) {
        getChannelServer().broadcastPacket(CWvsContext.getTopMsg(msg));
    }

    public void openNpc(int mapInstance, int npcID) {
        for (MapleCharacter user : getMapFactory().getMap(mapInstance).getAllCharactersThreadsafe()) {
            user.getClient().removeClickedNPC();
            NPCScriptManager.getInstance().start(user.getClient(), npcID);
        }
    }
