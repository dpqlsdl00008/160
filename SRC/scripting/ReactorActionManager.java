...
public class ReactorActionManager extends AbstractPlayerInteraction {
    ...
    public void dropItem(int... itemList) {
        ...
        for (int i = 0; i < itemList.length; i++) {
            ...
            if (GameConstants.getInventoryType(itemList[i]) != MapleInventoryType.EQUIP) {
                idrop = new Item(itemList[i], (byte) 0, (short) 1, (byte) 0);
            } else {
                idrop = ii.randomizeStats((Equip) ii.getEquipById(itemList[i]));
            }
            reactor.getMap().spawnItemDrop(reactor, getPlayer(), idrop, dropPos, false, false);
            dropPos.x += 24;
        }
    }
