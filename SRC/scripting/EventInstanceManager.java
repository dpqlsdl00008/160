...
public class EventInstanceManager {
    ...
    public void stopEventTimer() {
        ...
        for (MapleCharacter chr : getPlayers()) {
            chr.getClient().sendPacket(CField.stopClock());
        }
    }
            ...
            for (MapleCharacter chr : getPlayers()) {
                if (name.startsWith("PVP")) {
                    ...
                    if (MapConstants.isDeathCountMap(chr.getMapId()) != -1) {
                        chr.getClient().sendPacket(CField.getDeathCountClock(timesend));
                    } else {
    ...
    public void registerExpedition(MapleExpedition expedition, MapleMap map) {
        if (disposed) {
            return;
        }
        if (expedition == null) {
        ...
        for (int i : expedition.getParties()) {
            MapleParty party = World.Party.getParty(i);
            if (party != null) {
                for (MaplePartyCharacter pUser : party.getMembers()) {
                    if (pUser != null) {
                        registerPlayer(map.getCharacterById(pUser.getId()));
                    }
                }
            }
        }
    ...
    public void monsterDamaged(final MapleCharacter chr, final MapleMonster mob, final int damage) {
        //final List<Integer> mobs = Collections.unmodifiableList(Arrays.asList(9700037, 8850011));
    ...
    public void floatNotice(int mapid, String msg, int itemid) {
        this.getMapInstance(mapid).floatNotice(msg, itemid, false);
    }

    public void playSound(int mapid, String uol) {
        this.getMapInstance(mapid).broadcastMessage(FieldEffect.playSound(uol));
    }

    public void showFieldEffect(int mapid, String effect) {
        this.getMapInstance(mapid).broadcastMessage(CField.showEffect(effect));
    }
}
