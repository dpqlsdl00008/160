...
public class DumpItems {
    ...
    protected int id = 0;
    private Connection con = DatabaseConnection.getDumpConnection();
    ...
    public void dumpItems() throws Exception {
        if (!hadError) {
            ...
            PreparedStatement ps = con.prepareStatement("INSERT INTO wz_itemdata(itemid, name, msg, `desc`, slotMax, price, wholePrice, stateChange, flags, karma, meso, monsterBook, itemMakeLevel, questId, scrollReqs, consumeItem, totalprob, incSkill, replaceId, replaceMsg, `create`, afterImage, option1, option2, option3) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public void dumpItem(PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse, MapleData iz) throws Exception {
        ...
        } else if (GameConstants.isThrowingStar(id) || GameConstants.isBullet(id)) {
        ...
        if (dat != null) {
            ...
            for (final MapleData data : dat.getChildren()) {
                if (data.getName().startsWith("inc")) {
                    final long gg = MapleDataTool.getLongConvert(data, 0);
                    if (gg != 0) {
                        rett.put(data.getName().substring(3), (int) gg);
                    }
                }
            }
        ...
        ps.setInt(23, MapleDataTool.getInt("info/option/0/option", iz, 0));
        ps.setInt(24, MapleDataTool.getInt("info/option/1/option", iz, 0));
        ps.setInt(25, MapleDataTool.getInt("info/option/2/option", iz, 0));
        ps.addBatch();
    }
    //kinda inefficient
    ...
    protected final MapleData getStringData(final int itemId) {
        ...
        if (itemId >= 5010000) {
        ...
        } else if ((itemId >= 1132000 && itemId < 1191000) || (itemId >= 1010000 && itemId < 1040000) || (itemId >= 1122000 && itemId < 1123000)) {//183
            data = eqpStringData;
            cat = "Eqp/Accessory";
            ...
            data = eqpStringData;
            cat = "Eqp/Cap";
        } else if (itemId >= 1100001 && itemId < 1110000) {
            data = eqpStringData;
            cat = "Eqp/Cape";
            ...
            data = eqpStringData;
            cat = "Eqp/Glove";
        } else if ((itemId >= 30000 && itemId < 50000) || (itemId >= 60000 && itemId < 70000)) {
            data = eqpStringData;
            cat = "Eqp/Hair";
