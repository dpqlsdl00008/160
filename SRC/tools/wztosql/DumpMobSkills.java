	...
	public void dumpMobSkills(PreparedStatement ps) throws Exception {
		...
		for (MapleData ids : skillz.getChildren()) {
			for (MapleData lvlz : ids.getChildByPath("level").getChildren()) {
				...
				if (lvlz.getChildByPath("mobGroup") != null) {
                                    for (int i = 0; i > -1; i++) {
                                        if (lvlz.getChildByPath("mobGroup/"+String.valueOf(i)) == null) {
                                            break;
                                        }
                                        for (int c = 0; c > -1; c++) {
                                            if (lvlz.getChildByPath("mobGroup/"+String.valueOf(i)+"/"+String.valueOf(c)) == null) {
                                                break;
                                            }
                                            toSummon.add(Integer.valueOf(MapleDataTool.getInt(lvlz.getChildByPath("mobGroup/"+String.valueOf(i)+"/"+String.valueOf(c)), 0)));
                                        }
                                    }
                                }                                
