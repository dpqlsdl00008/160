 ...
 * @author 몽키프
 */
public class WinterLimitedExtractor {
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException {
        ...
        int count = 1;
        String item_string = "";
        System.out.println("[프로스티무기 추출기 @author 몽키프]\r\n");
        System.out.println("[프로스티무기 추출한당]");
        ...
        for (MapleData ddd : pData.getChildren()) {
            for (MapleData dd : ddd) {
                ...
                if (dd.getName().equals("name")) {
                    ...
                    if (MapleDataTool.getString("name", ddd).contains("의 신비한 마스터리") && !겹치지마.contains(MapleDataTool.getString("name", ddd))) {
                        if (!first) {
                            mh.append(",").append("/*").append(item_string).append("*/").append("\r\n");
                        }
                        mh.append("(307, ").append(ddd.getName()).append(", ").append(0).append(", ").append(count).append(", 4310066, 50, 7)");
        ...
        */
        ...
        FileOutputStream cash_string = null;
        cash_string = new FileOutputStream("WinterLimited.txt");
        cash_string.write(mh.toString().getBytes());
        cash_string.close();
