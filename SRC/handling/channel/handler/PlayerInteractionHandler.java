    ...
    public static final void PlayerInteraction(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) throws Exception {
        ...
        switch (action) { // Mode
            case CREATE: {
                ...
                if (createType == 3) { // trade
                    ...
                    if (createType == 1 || createType == 2) {
                        ...
                        MapleMiniGame game = new MapleMiniGame(chr, itemId, desc, pw, createType); //itemid
                        ...
                        if (createType == 4) {
                            ...
                            c.getSession().write(CPlayerShop.getPlayerStore(chr, true));
                            ...
                            c.getPlayer().getClient().getSession().write(CPlayerShop.getHiredMerch(chr, merch, true));
                        }
                    }
                }
                break;
                        ...
                        if (ob instanceof HiredMerchant) {
                            ...
                            if (ips instanceof MaplePlayerShop && ((MaplePlayerShop) ips).isBanned(chr.getName())) {
                                ...
                                if (ips.getFreeSlot() < 0 || ips.getVisitorSlot(chr) > -1 || !ips.isOpen() || !ips.isAvailable()) {
                                    c.getSession().write(CPlayerShop.getMiniGameFull());
                                } else {
                                    ...
                                    System.out.print("고추있는에녹");
                                    chr.setPlayerShop(ips);
                                    ips.addVisitor(chr);
                                    if (ips instanceof MapleMiniGame) {
                                        ((MapleMiniGame) ips).send(c);
                                    } else {
                                        c.getSession().write(CPlayerShop.getPlayerStore(chr, false));
                                    }
                                }
                            }
                        }
            ...
            case HIRED_MERCHANT_MAINTENANCE: {
                if (c.getChannelServer().isShutdown() || chr.getMap() == null) { // 교환 널
                    c.getSession().write(CWvsContext.enableActions());
                    return;
                }
                byte type = slea.readByte();
                byte tradetype = slea.readByte();
                //if (type != 6) {
                //    c.getSession().write(CWvsContext.enableActions());
                //    return;
                //} [R] DA 00 14 06 07 04 00 20 20 20 20 66 00 00 00
                ...
                if (type == 6 && tradetype == 6) {
                    MapleCharacter chrr = chr.getMap().getCharacterById_InMap(obid); //BC 07 00 00 // 대상인가? 아이디
                    if (chrr == null || c.getChannelServer().isShutdown() || chrr.hasBlockedInventory()) {
                        c.getSession().write(CWvsContext.enableActions());
                        return;
                    }
                    MapleTrade.startCashTrade(chr); //추가필요부분                
                    MapleTrade.inviteCashTrade(chr, chrr);
                } else if (tradetype != 6 && type != 0x10) {
                    MapleMapObject ob = chr.getMap().getMapObject(obid, MapleMapObjectType.HIRED_MERCHANT);
                    ...
                    if (ob instanceof IMaplePlayerShop && ob instanceof HiredMerchant) {
                        ...
                        if (merchant.isOwner(chr) && merchant.isOpen() && merchant.isAvailable()) {
                            merchant.setOpen(false);
                            merchant.removeAllVisitors((byte) 6, (byte) 1);
                            chr.setPlayerShop(ips);
                            c.getSession().write(CPlayerShop.getHiredMerch(chr, merchant, false));
                        } else {
                            c.getSession().write(CWvsContext.enableActions());
                    ...
                    if (shop != null && shop.isOwner(chr) && shop.getShopType() < 3 && !shop.isAvailable()) {
                        if (chr.getMap().allowPersonalShop()) {
                            ...
                            if (shop.getShopType() == 1 && HiredMerchantHandler.UseHiredMerchant(chr.getClient(), false)) {
                                ...
                                chr.getMap().broadcastMessage(CPlayerShop.spawnHiredMerchant(merchant));
                                chr.setPlayerShop(null);
                                c.getSession().write(CPlayerShop.MerchantClose(0, 5));
                    ...
                    MapleTrade.visitCashTrade(chr, chr.getTrade().getPartner().getChr());      //이건 수락받기 인데.      
                }
                break;
            }
            case CHAT: {
                ...
                if (chr.getTrade() != null) {
                    ...
                    ips.broadcastToVisitors(CPlayerShop.shopChat(chr.getName() + " : " + message, ips.getVisitorSlot(chr)));
            ...
            case ADD_ITEM: {
                ...
                if (ivItem != null) {
                    ...
                    if (check > 99999 || check <= 0) { //This is the better way to check.
                        return;
                    }
...
//                    }
                    if (ivItem.getQuantity() >= bundles_perbundle) {
                        ...
                        if (ItemFlag.Untradable.check(flag) || ItemFlag.Locked.check(flag)) {
                            c.getSession().write(CWvsContext.enableActions());
                            return;
                        }
                        if (ii.isDropRestricted(ivItem.getItemId()) || ii.isAccountShared(ivItem.getItemId())) {
                            if (!(ItemFlag.KarmasCissors.check(flag) || ItemFlag.PreventSlipping.check(flag))) {
                                c.getSession().write(CWvsContext.enableActions());
                                return;
                            }
                        }
                        ...
                        c.getSession().write(CPlayerShop.shopItemUpdate(shop));
                    }
                }
                break;
            }
            ...
            case BUY_ITEM_HIREDMERCHANT: { // Buy and Merchant buy
                ...
                if (check <= 0 || check2 > 2147483647 || check2 <= 0 || check3 > 99999 || check3 < 0) { //This is the better way to check.
                    return;
                }
                if (tobuy.bundles < quantity
                        || (tobuy.bundles % quantity != 0 && GameConstants.isEquip(tobuy.item.getItemId()))
                        //|| chr.itemQuantity(4310114) - (check2) < 0
                        //|| chr.itemQuantity(4310114) - (check2) > 2147483647
                        || shop.getMeso() + (check2) < 0
                        || shop.getMeso() + (check2) > 2147483647) {
                    //chr.dropMessage(1, "메소 동전이 충분하지 않습니다.");
                ...
                shop.broadcastToVisitors(CPlayerShop.shopItemUpdate(shop));
                break;
            }
            case REMOVE_ITEM: {
                slea.skip(1); // ?
            ...
            case MAINTANCE_ORGANISE: {
                ...
                if (imps != null && imps.isOwner(chr) && !(imps instanceof MapleMiniGame)) {
                    ...
                    /* 메소 동전 */
                    if (chr.itemQuantity(4310114) + imps.getMeso() > 0) {
                        chr.gainMeso(imps.getMeso(), false);
                        
                        //chr.gainItem(4310114, imps.getMeso());
                        //chr.getClient().sendPacket(CWvsContext.InfoPacket.getShowItemGain(4310114, imps.getMeso(), true));
                        
                        imps.setMeso(0);
                    }
                    c.getSession().write(CPlayerShop.shopItemUpdate(imps));
                }
                break;
            }
            case CLOSE_MERCHANT: {
            ...
            case GIVE_UP: {
                ...
                if (ips != null && ips instanceof MapleMiniGame) {
                    ...
                    game.broadcastToVisitors(CPlayerShop.getMiniGameResult(game, 0, game.getVisitorSlot(chr)));
                    game.nextLoser();
                    game.setOpen(true);
                    game.update();
            ...
            case UN_READY: {
                ...
                if (ips != null && ips instanceof MapleMiniGame) {
                    ...
                    if (!game.isOwner(chr) && game.isOpen()) {
                        ...
                        game.broadcastToVisitors(CPlayerShop.getMiniGameReady(game.isReady(game.getVisitorSlot(chr))));
                    }
                }
                break;
            }
                ...
                if (ips != null && ips instanceof MapleMiniGame) {
                    ...
                    if (game.isOwner(chr) && game.isOpen()) {
                        ...
                        if (game.getGameType() == 1) {
                            game.broadcastToVisitors(CPlayerShop.getMiniGameStart(game.getLoser()));
                        } else {
                            game.broadcastToVisitors(CPlayerShop.getMatchCardStart(game, game.getLoser()));
                        }
                        game.setOpen(false);
                        game.update();
                    }
            ...
            case REQUEST_TIE: {
                ...
                if (ips != null && ips instanceof MapleMiniGame) {
                    ...
                    if (game.isOwner(chr)) {
                        game.broadcastToVisitors(CPlayerShop.getMiniGameRequestTie(), false);
                    } else {
                        game.getMCOwner().getClient().getSession().write(CPlayerShop.getMiniGameRequestTie());
                    }
            ...
            case ANSWER_TIE: {
                ...
                if (ips != null && ips instanceof MapleMiniGame) {
                    ...
                    if (game.getRequestedTie() > -1 && game.getRequestedTie() != game.getVisitorSlot(chr)) {
                        if (slea.readByte() > 0) {
                            game.broadcastToVisitors(CPlayerShop.getMiniGameResult(game, 1, game.getRequestedTie()));
                            game.nextLoser();
                            game.setOpen(true);
                            game.update();
                            game.checkExitAfterGame();
                        } else {
                            game.broadcastToVisitors(CPlayerShop.getMiniGameDenyTie());
                        }
            ...
            case REQUEST_REDO: {
                ...
                if (ips != null && ips instanceof MapleMiniGame) {
                    ...
                    if (game.isOwner(chr)) {
                        game.broadcastToVisitors(CPlayerShop.getMiniGameRequestRedo(), false);
                    } else {
                        game.getMCOwner().getClient().getSession().write(CPlayerShop.getMiniGameRequestRedo());
                    }
                }
                break;
            }
            case ANSWER_REDO: {
                c.getPlayer().dropMessage(1, "못무름ㅋ.");
                /*final IMaplePlayerShop ips = chr.getPlayerShop();
                if (ips != null && ips instanceof MapleMiniGame) {
                    MapleMiniGame game = (MapleMiniGame) ips;
                    if (game.isOpen()) {
            ...
            case SKIP: {
                ...
                if (ips != null && ips instanceof MapleMiniGame) {
                    ...
                    ips.broadcastToVisitors(CPlayerShop.getMiniGameSkip(ips.getVisitorSlot(chr) == 0 ? 1 : 0));
                    game.nextLoser();
                    break;
                }
            }
            ...
            case SELECT_CARD: {
                ...
                if (ips != null && ips instanceof MapleMiniGame) {
                    ...
                    if (turn == 1) {
                        ...
                        if (game.isOwner(chr)) {
                            game.broadcastToVisitors(CPlayerShop.getMatchCardSelect(turn, slot, fs, turn), false);
                        } else {
                            game.getMCOwner().getClient().getSession().write(CPlayerShop.getMatchCardSelect(turn, slot, fs, turn));
                        }
                        game.setTurn(0); //2nd turn nao
                        return;
                    } else if (fs > 0 && game.getCardId(fs + 1) == game.getCardId(slot + 1)) {
                        game.broadcastToVisitors(CPlayerShop.getMatchCardSelect(turn, slot, fs, game.isOwner(chr) ? 2 : 3));
                        game.setPoints(game.getVisitorSlot(chr)); //correct.. so still same loser. diff turn tho
                    } else {
                        game.broadcastToVisitors(CPlayerShop.getMatchCardSelect(turn, slot, fs, game.isOwner(chr) ? 0 : 1));
            ...
            case CANCEL_EXIT: {
                ...
                if (ips != null && ips instanceof MapleMiniGame) {
                    ...
                    game.broadcastToVisitors(CPlayerShop.getMiniGameExitAfter(game.isExitAfter(chr)));
                }
                break;
            }
            default: {
    ...
    private static void CloseMerchant(final MapleCharacter chr, final MapleClient c) {
        ...
        if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr) && merchant.isAvailable()) {
            chr.dropMessage(1, "프레드릭에게 가서 아이템을 수령하세요.");
            c.sendPacket(CWvsContext.enableActions());
            merchant.removeAllVisitors(-1, -1);
            merchant.closeShop(true, true);
            c.sendPacket(CPlayerShop.MerchantClose(0, 5));
            chr.setPlayerShop(null);
