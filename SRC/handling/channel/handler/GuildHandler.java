...
public class GuildHandler {
    ...
    public static final void DenyGuildRequest(final String from, final MapleClient c) {
        final MapleCharacter cfrom = ChannelServer.getInstance(World.Find.findChannel(from)).getPlayerStorage().getCharacterByName(from);
        ...
        switch (slea.readByte()) {
            case 0x02:
                if (c.getPlayer().getGuildId() > 0) {
                    ...
                    c.getPlayer().dropMessage(1, "길드 제작에 필요한 메소 [50만 메소] 가 충분하지 않습니다.");
                    return;
                }
                final String guildName = slea.readMapleAsciiString();

                if (!isGuildNameAcceptable(guildName)) {
                    c.sendPacket(CWvsContext.OnGuildResult((byte) 1));
                    return;
                }
                ...
                c.getSession().write(NPCPacket.getNPCTalk(2010007, (byte) 0, "\r\n축하하네~ #e#b" + c.getPlayer().getGuild().getName() + "#k#n가 우리 길드 사무소에 새로 등록되었네... 앞으로 서로 도와가며 더 많은 성장이 있기를 바라네~", "00 01", (byte) 0));
