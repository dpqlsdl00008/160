...
public class SummonHandler {
        ...
        try {
            while (iter.hasNext()) {
                ...
                if (summon.isPuppet() && summon.getOwnerId() == chr.getId() && damage > 0) {
                    int newdamage = (damage / 100);
                    summon.addHP((short) -newdamage);
                    if (summon.getHP() <= 0) {
                        remove = true;
                    }
                    chr.getMap().broadcastMessage(chr, SummonPacket.damageSummon(chr.getId(), summon.getSkill(), newdamage, unkByte, monsterIdFrom), summon.getTruePosition());
                    break;
                }
            }
        } finally {
    ...
    public static void SummonAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        AttackInfo attack = DamageParse.parseDmgSummon(slea, chr);
        final MapleMap map = chr.getMap();
        final MapleMapObject obj = map.getMapObject(attack.skill, MapleMapObjectType.SUMMON);
        final MapleSummon summon = (MapleSummon) obj;
        map.broadcastMessage(chr, SummonPacket.summonAttack(summon.getOwnerId(), summon.getObjectId(), (byte) attack.display, attack.allDamage, chr.getLevel(), attack.tbyte, false), summon.getTruePosition());
        ...
        for (AttackPair oned : attack.allDamage) {
            ...
            for (Pair eachde : oned.attack) {
                ...
                toDamage += eachd;
                if (((eachd == 0) || (monster.getId() == 9700021)) && (chr.getPyramidSubway() != null)) {
                    chr.getPyramidSubway().onMiss(chr);
                }
            }
            if (toDamage > 0 && summonEffect.getMonsterStati().size() > 0) {
                if (summonEffect.makeChanceResult()) {
                    for (Map.Entry<MonsterTemporaryStat, Integer> z : summonEffect.getMonsterStati().entrySet()) {
                        monster.applyStatus(chr, new MonsterTemporaryStatEffect(z.getKey(), z.getValue(), summonSkill.getId(), null, false), summonEffect.isPoison(), 4000, true, summonEffect);
                    }
                }
            }
            monster.damage(chr, toDamage, true);
            chr.checkMonsterAggro(monster);
            if (!monster.isAlive()) {
                chr.getMap().broadcastMessage(CMobPool.killMonster(monster.getObjectId(), 1));
            }
        }
    ...
    public static final void RemoveSummon(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        final MapleMapObject obj = c.getPlayer().getMap().getMapObject(objectID, MapleMapObjectType.SUMMON);
        ...
        c.getPlayer().removeSummonList(summon);
        if (summon.getSkill() != 35121011) {
    ...
    public static final void SubSummon(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        switch (sum.getSkill()) {
            ...
            case 1321007: //beholder
                ...
                if (bHealing.getId() == 1320009) {
                    ...
                    if (buffid == 2022125) {
                        localstatups.put(CharacterTemporaryStat.EnhancedPdd, healEffect.getEnhancedWdef());
                    } else if (buffid == 2022126) {
                        localstatups.put(CharacterTemporaryStat.EnhancedMdd, healEffect.getEnhancedMdef());
                    } else if (buffid == 2022127) {
                        localstatups.put(CharacterTemporaryStat.Acc, healEffect.getAcc());
                    } else if (buffid == 2022128) {
                        localstatups.put(CharacterTemporaryStat.Eva, healEffect.getAvoid());
                    } else if (buffid == 2022129) {
                        localstatups.put(CharacterTemporaryStat.EnhancedPad, healEffect.getEnhancedWatk());
                    }
                    chr.cancelEffect(stateff, -1);
                    chr.getClient().getSession().write(CWvsContext.BuffPacket.giveBuff(-buffid, healEffect.getDuration(), localstatups, stateff));
                    ...
                    chr.registerEffect(stateff, starttime, schedule, chr.getId());
        ...
        if (GameConstants.isAngel(sum.getSkill())) {
            ...
            chr.getClient().getSession().write(EffectPacket.showOwnBuffEffect(sum.getSkill() % 10000, 2, 2, 1));
            chr.getMap().broadcastMessage(chr, EffectPacket.showBuffeffect(chr.getId(), sum.getSkill() % 10000, 2, 2, 1), false);
        }
    }
    ...
    public static final void SummonPVP(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        for (MapleMapObject mo : chr.getMap().getCharactersIntersect(box)) {
            ...
            if (attacked.getId() != chr.getId() && attacked.isAlive() && !attacked.isHidden() && (type == 0 || attacked.getTeam() != chr.getTeam())) {
                ...
                if (attacked.getBuffedValue(CharacterTemporaryStat.PvPDamage) != null || PlayersHandler.inArea(attacked)) {
                    rawDamage = 0;
                }
