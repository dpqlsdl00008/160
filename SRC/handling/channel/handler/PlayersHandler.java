...
import tools.packet.CNettPyramid;
...
public class PlayersHandler {
    ...
    public static void GiveFame(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (chr.canGiveFame(target)) {
            case OK:
                ...
                if (target.getMapId() == 910000000) {
                    if (mode != 0) {
                        MapleItemInformationProvider.getInstance().getItemEffect(2023129).applyTo(c.getPlayer());
                        c.getPlayer().getClient().sendPacket(CUserLocal.chatMsg(ChatType.GameDesc, target.getName() + "님께서 [인기인의 선물] 효과를 시전하였습니다."));
                    }
                }
                break;
            case NOT_TODAY:
    ...
    public static void TouchReactor(final LittleEndianAccessor slea, final MapleClient c) {
        final int oid = slea.readInt();
        boolean touched = slea.available() == 0;
        boolean v1 = false;
        if (slea.available() > 0) {
            v1 = true;
            touched = slea.available() == 0 || slea.readByte() > 0;
        }
        final MapleReactor reactor = c.getPlayer().getMap().getReactorByOid(oid);
        if (!touched || reactor == null || !reactor.isAlive() || (v1 && reactor.getTouch() == 0)) {
            return;
        }
    ...
    public static void hitCoconut(LittleEndianAccessor slea, MapleClient c) {
        ...
        String co = "coconut";
        MapleCoconut map = (MapleCoconut) c.getChannelServer().getEvent(MapleEventType.코코넛);
        if (map == null || !map.isRunning()) {
            map = (MapleCoconut) c.getChannelServer().getEvent(MapleEventType.코크);
            co = "coke cap";
            if (map == null || !map.isRunning()) {
                return;
            }
        ...
        if (c.getPlayer().getFollowId() > 0 && c.getPlayer().getFollowId() == slea.readInt()) {
            ...
            if (tt != null && tt.getPosition().distanceSq(c.getPlayer().getPosition()) < 10000 && tt.getFollowId() == 0 && tt.getId() != c.getPlayer().getId()) { //estimate, should less
                ...
                if (accepted) {
                    ...
                    tt.setFollowId(0);
                    tt.getClient().sendPacket(CUserLocal.applyFollowResult(0, 5));
                }
            } else {
                if (tt != null) {
                    tt.setFollowId(0);
    ...
    public static void OnClaimResult(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (other == null || type == null || other.isIntern()) {
            c.getSession().write(CWvsContext.OnClaimResult(4));
            return;
        }
        ...
        if (theTime + 7200000 > currentTime && !c.getPlayer().isIntern()) {
            ...
            c.getSession().write(CWvsContext.OnClaimResult(2));
        }
    }
    ...
    public static final void ChangeSet(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getMonsterBook().changeSet(set)) {
            ...
            c.getSession().write(CWvsContext.MonsterBookSetCover(set));
        }
    }
    ...
    public static final void EnterPVP(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        for (MaplePartyCharacter pMember : user.getParty().getMembers()) {
            if (pMember != null) {
                MapleCharacter cUser = c.getChannelServer().getMapFactory().getMap(user.getMapId()).getCharacterByName(pMember.getName());
                if (cUser != null) {
                    cUser.updateOneInfoQuest(nQRecord, "karing_time", (System.currentTimeMillis() + 30 * 60000) + "");
                }
            }
        }
        EventManager eScript = c.getChannelServer().getEventSM().getEventManager("expedition_karing");
        if (eScript != null) {
            eScript.setProperty("map", nMap + "");
            eScript.startInstance_Party(nMap + "", user);
        }
        /*
        if (c.getPlayer() == null || c.getPlayer().getMap() == null || c.getPlayer().getMapId() != 960000000) {
            //c.getSession().write(CField.pvpBlocked(1));
            c.getPlayer().dropMessage(1, "An error has occurred. Please try again later. 1");
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        if (c.getPlayer().getParty() != null) {
            //c.getSession().write(CField.pvpBlocked(9));
            c.getPlayer().dropMessage(1, "Parties are not supported at this time.");
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        c.getPlayer().updateTick(slea.readInt());
        ...
        if (!passed || em == null) {
            //c.getSession().write(CField.pvpBlocked(1));
            //c.getPlayer().dropMessage(1, "An error has occurred. Please try again later. 2");
            //c.getSession().write(CWvsContext.enableActions());
            //return;
        }
        ...
        if (!passed) {
            c.getPlayer().dropMessage(1, "An error has occurred. Please try again later. 3");
            //c.getSession().write(CField.pvpBlocked(1));
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
    ...
    public static final void LeavePVP(final LittleEndianAccessor slea, final MapleClient c) {
        if (c.getPlayer() == null || c.getPlayer().getMap() == null || !c.getPlayer().inPVP()) {
            ...
            c.getPlayer().dropMessage(1, "An error has occurred. Please try again later. 4 ");
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        ...
        c.getPlayer().changeMap(c.getChannelServer().getMapFactory().getMap(960000000), c.getChannelServer().getMapFactory().getMap(960000000).getPortal(1));
    ...
    public static final void EnterAzwan(final LittleEndianAccessor slea, final MapleClient c) {
        if (c.getPlayer() == null || c.getPlayer().getMap() == null || c.getPlayer().getMapId() != 262000300) {
            c.getPlayer().dropMessage(MessageType.POPUP, "System error.");
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        if (c.getPlayer().getLevel() < 40) {
            c.getPlayer().dropMessage(MessageType.POPUP, "You must be at least Level 40 to attempt Azwan.");
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        ...
        int mapid = 262020000 + (mode * 1000) + difficult;

        if (mode != 3) {
            c.getPlayer().dropMessage(MessageType.POPUP, "This feature is currently disabled. \r\nOnly Supply Mode is currently available.");
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        if (party == 1 && c.getPlayer().getParty() == null) {
            //c.getSession().write(CField.pvpBlocked(9));
            c.getPlayer().dropMessage(MessageType.POPUP, "You are not in a party.");
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        if (party == 1 && c.getPlayer().getParty() != null) {
        ...
        if (mode == 3) {
            ...
            EventManager em = c.getChannelServer().getEventSM().getEventManager("AzwanSupply");

            if (em == null) {
                c.getPlayer().dropMessage(1, "The EventManager is null, report it to Cloud!");
                c.getSession().write(CWvsContext.enableActions());
                return;
            }

            EventInstanceManager eim = em.newInstance("AzwanSupply");
            String eim_status = em.getProperty("state");
            //Checks to see if it's currently in proccess, if not, create a new one.
            if (eim_status == null || eim_status.equals("0")) {

                //Set the properties
                MapleMap map = eim.setInstanceMap(mapid);
                eim.setProperty("Global_StartMap", mapid + "");
                eim.setProperty("Global_MinPerson", "1");
                eim.setProperty("Global_ExitMap", "262000200");

                //Prepare the timer.
                eim.startEventTimer(3600000);

                //Prepare the mobs, adjust according to level.
                map.killAllMonsters(false);
                map.respawn(true);

                //Doing Party Supply Mode
                if (party == 1 && c.getPlayer().getParty() != null) {
                    //Sets aggro?
                    ...
                    //Get highest level of the party.
                    int highestLevel = 0;
                    for (MaplePartyCharacter partyChar : c.getPlayer().getParty().getMembers()) {
                        if (partyChar.getLevel() >= highestLevel) {
                            highestLevel = partyChar.getLevel();
                        }
                    }

                    //Adjust mob stats to party's highest level.
                    for (MapleMonster mm : map.getAllMonstersThreadsafe()) {
                        mm.hellChangeLevel(highestLevel - 10, 1, 1, 1.5);
                    }

                    //Register people for the event.
                    eim.registerPlayer(c.getPlayer());
                    for (MaplePartyCharacter partyChar : c.getPlayer().getParty().getMembers()) {
                        eim.registerPlayer(c.getChannelServer().getPlayerStorage().getCharacterById(partyChar.getId()));
                        c.getSession().write(CWvsContext.enableActions());
                    }

                    //Start the instance.
                    em.startInstance_Party(mapid + "", c.getPlayer());
                    //Finally, warp them to the map.
                    for (MaplePartyCharacter partymembers : c.getPlayer().getParty().getMembers()) {
                        c.getChannelServer().getPlayerStorage().getCharacterById(partymembers.getId()).changeMap(map);
                    }

                } else {//Otherwise, single person; not in a party.

                    //Sets aggro?
                    map.setChangeableMobOrigin(c.getPlayer());

                    //Adjust mob stats to players level.
                    for (MapleMonster mm : map.getAllMonstersThreadsafe()) {
                        mm.hellChangeLevel(c.getPlayer().getLevel() - 10, 1, 1, 1.5);
                    }

                    //Register the person for the event.
                    eim.registerPlayer(c.getPlayer());

                    //Start the instance.
                    em.startInstance(c.getPlayer());
                    c.getPlayer().changeMap(map);
                }
            } else {
                c.getPlayer().dropMessage(MessageType.ERROR, "Sorry, it looks like a battle is already taking place!");
            }
        }
    }
    ...
    public static final void LeaveAzwan(final LittleEndianAccessor slea, final MapleClient c) {
        if (c.getPlayer() == null || c.getPlayer().getMap() == null || !c.getPlayer().inAzwan()) {
            //c.getSession().write(CField.pvpBlocked(6));
            c.getPlayer().dropMessage(1, "An error has occurred. Please try again later.");
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        c.getPlayer().cancelAllBuffs();
        ...
        c.getPlayer().dispelDebuffs();
        c.getPlayer().clearAllCooldowns();
        c.sendPacket(CWvsContext.offStaticScreenMessage());
    ...
    public static final void viewSkills(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (!c.getChannelServer().getPlayerStorage().getCharacterById(victim).getSkills().isEmpty() && GameConstants.isAdventurer(jobid)) {
            ...
            c.getPlayer().dropMessage(6, "타겟이 모험가가 아닙니다.");
        }
    }
    ...
    public static final void AttackPVP(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        int addedScore = 0, skillLevel = 0, trueSkillLevel = 0, animation = -1, attackCount = 1, mobCount = 1, fakeMastery = chr.getStat().passive_mastery(), ignoreDEF = (int) chr.getStat().ignoreTargetDEF, critRate = chr.getStat().passive_sharpeye_rate(), skillDamage = 100;
        ...
        if (skillid > 0) {
            ...
            if (effect.getCooldown(chr) > 0 && !chr.isGM()) {
                ...
                if ((skillid != 35111004 && skillid != 35121013) || chr.getBuffSource(CharacterTemporaryStat.Mechanic) != skillid) {
        ...
        if (!area) {
            ...
            for (MapleCharacter attacked : chr.getMap().getCharactersIntersect(box)) {
                if (attacked.getId() != chr.getId() && attacked.isAlive() && !attacked.isHidden() && (type == 0 || attacked.getTeam() != chr.getTeam())) {
                    ...
                    if (attacked.getBuffedValue(CharacterTemporaryStat.PvPDamage) != null || inArea(attacked)) {
                        rawDamage = 0;
                    }
                    ...
                    try {
                        for (int i = 0; i < attackCount; i++) {
                            ...
                            if (attacked.getStat().evaR > 0 && Randomizer.nextInt(100) < attacked.getStat().evaR) {
                                ourDamage = 0;
                            } else if (attacked.hasDisease(MonsterSkill.Darkness) && Randomizer.nextInt(100) < 50) {
                                ourDamage = 0;
                            ...
                            if (ourDamage > 0 && skillid != 62111002 && skillid != 4211006 && skillid != 3211003 && skillid != 4111004 && (skillid == 4221001 || skillid == 3221007 || skillid == 23121003 || skillid == 4341005 || skillid == 4331006 || skillid == 21120005 || Randomizer.nextInt(100) < critRate)) {
                    ...
                    if (GameConstants.isDemon(chr.getJob()) && !GameConstants.isDemonAvenger(chr.getJob())) {
                    ...
                    if (effect != null) {
                        if (effect.getMonsterStati().size() > 0 && effect.makeChanceResult()) {
                            ThreadLock.lock();
                            try {
                                for (Map.Entry<MonsterTemporaryStat, Integer> z : effect.getMonsterStati().entrySet()) {
                                    MonsterSkill d = MonsterTemporaryStat.getAffectedCTS(z.getKey());
                                    if (d != null) {
                                        attacked.giveDebuff(d, z.getValue(), effect.getDuration(), d.getDisease(), 1, 0);
                                    }
                                }
                            } finally {
                                ThreadLock.unlock();
                    ...
                    if (chr.getJob() == 121 || chr.getJob() == 122 || chr.getJob() == 2110 || chr.getJob() == 2111 || chr.getJob() == 2112) { // WHITEKNIGHT
                        if (chr.getBuffSource(CharacterTemporaryStat.WeaponCharge) == 1211006 || chr.getBuffSource(CharacterTemporaryStat.WeaponCharge) == 21101006) {
                            ...
                            if (eff.makeChanceResult()) {
                                attacked.giveDebuff(MonsterSkill.Frozen, 1, eff.getDuration(), MonsterSkill.Frozen.getDisease(), 1, 0);
                            }
                        }
                        ...
                        if (eff != null && eff.makeChanceResult()) {
                            attacked.giveDebuff(MonsterSkill.Slow, 100 - Math.abs(eff.getX()), eff.getDuration(), MonsterSkill.Slow.getDisease(), 1, 0);
                        }
                            ...
                            for (int i : skills) {
                                ...
                                if (chr.getTotalSkillLevel(skill) > 0) {
                                    ...
                                    if (venomEffect.makeChanceResult()) {// THIS MIGHT ACTUALLY BE THE DOT
                                        attacked.giveDebuff(MonsterSkill.Poison, 1, venomEffect.getDuration(), MonsterSkill.Poison.getDisease(), 1, 0);
                                    }
                                    break;
                                }
                            }
    ...
    public static void GemAutoBuff(LittleEndianAccessor slea, MapleClient c) {
        //B0 00 [Header]
        //00 00 00 00 [unk]
        //6A 88 1E 00 [skillid]
        //00 [unk]
    ...
    public static void handleBuffProtector(LittleEndianAccessor slea, MapleClient c) {
        final MapleCharacter user = c.getPlayer();
        int checkfreezer = user.getItemQuantity(5133000, false);
        if (checkfreezer >= 1) {

            //user.getStat().setHp(50, user);
            //user.updateSingleStat(MapleStat.HP, user.getStat().getHp());
            //user.setSoulStone(false);
            c.sendPacket(CUserLocal.setBuffProtector(5133000));
            MapleInventoryManipulator.removeById(c, MapleInventoryType.CASH, 5133000, 1, true, false);
        }
    }

    public static void link_skill(LittleEndianAccessor slea, MapleClient c) {
        int skillv = slea.readInt();
        int linked_charId = slea.readInt();
        int linking_charId = c.getPlayer().getId();
        MapleCharacter lchar = MapleCharacter.loadCharFromDB(linked_charId, c, false);
        int skillid = -1;
        switch (skillv) {
            case 110:
                skillid = 80000000;
        ...
        if (skillid < 0) {
            c.getSession().write(CWvsContext.LinkedSkillAction(skillv, "", "", linked_charId, 2));
            return;
        }
        if (c.getPlayer().getLevel() < 70) {
            c.getPlayer().dropMessage(1, "아직 레벨 70이 되지 않아서, 스킬을 전수해줄 캐릭터를 선택할 수 없습니다.");
            return;
        }
        if (c.alreadyLinkingv(linked_charId, skillid)) {
            int alreadLinked_charId = c.getOchrId(linked_charId, skillid);
            MapleCharacter Achar = MapleCharacter.loadCharFromDB(alreadLinked_charId, c, false);
            c.getSession().write(CWvsContext.LinkedSkillAction(skillv, lchar.getName(), Achar.getName(), linked_charId, 1));
            return;
        }
        /*
        if (!c.canChange(linking_charId, skillid)) {
            c.getPlayer().dropMessage(1, "캐릭터 선택 후 하루동안은 캐릭터를 변경하실 수 없습니다.");
            return;
        }
         */
        if (!c.alreadyLinked(linked_charId, skillid)) {
            c.getPlayer().changeSingleSkillLevel(SkillFactory.getSkill(skillv), c.getPlayer().getSkillLevel(skillv), (byte) SkillFactory.getSkill(skillv).getMasterLevel());
            c.getSession().write(CWvsContext.LinkedSkillAction(skillv, lchar.getName(), "", linked_charId, 0));
            c.addLinkSkill(linked_charId, linking_charId, skillid, 60 * 60 * 60 * 24 * 1000, System.currentTimeMillis());
        }
        c.getPlayer().getStat().recalcLocalStats(c.getPlayer());
    }

    public static void DoubleDownUnk(LittleEndianAccessor slea, MapleClient c) {
        //This is sent to the server after gaining a double down buff and after one expires or is cancelled.
        /* 153 Packet struct:
         * [53 01]//Header
         * [51 45 FD 44]//Timestamp 
         * [00] unk byte 
         */
        throw new UnsupportedOperationException("Not supported yet.");
    }
    ...
    public static void LieDetector(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr, final boolean isItem) { // Person who used 
        ...
        } else if (!chr.isGM() && chr.getJob() != 800) {
            c.getSession().close();
            return;
        }
        ...
        if (search_chr == null || search_chr.getId() == chr.getId() || search_chr.isGM() && !chr.isGM()) {
            c.sendPacket(CWvsContext.LieDetector(null, (byte) 0, (byte) 0, null));
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        if (search_chr.getAntiMacro().canstartLie(System.currentTimeMillis())) {
            search_chr.getAntiMacro().passed = false;
        }
        if (search_chr.getEventInstance() != null) {
            c.sendPacket(CWvsContext.LieDetector(null, (byte) 1, (byte) 0, null));
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        if (search_chr.getAntiMacro().inProgress()) {
            c.sendPacket(CWvsContext.LieDetector(null, (byte) 3, (byte) 0, null));
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        if (search_chr.getAntiMacro().isPassed() && isItem || search_chr.getAntiMacro().getAttempt() == 2) {
            c.sendPacket(CWvsContext.LieDetector(null, (byte) 2, (byte) 0, null));
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        if (!search_chr.getAntiMacro().startLieDetector(chr.getName(), isItem, false)) {
            chr.dropMessage(5, "지금은 거짓말 탐지기를 사용할 수 없으므로 다음에 다시 시도해주세요.");
            c.sendPacket(CWvsContext.enableActions());
            return;
        } else {
            chr.getAntiMacro().startLie = System.currentTimeMillis();
        }
        if (isItem) {
            MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (short) 1, false);
        }
        chr.getClient().sendPacket(CWvsContext.LieDetector(search_chr.getName(), (byte) 6, (byte) 1, null));
        //c.sendPacket(CWvsContext.enableActions());
    }

    public static void LieDetectorResponse(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        if (c.getPlayer() == null || c.getPlayer().getMap() == null) {
            return;
        }
        ...
        if (answer.toLowerCase().equals(ld.getAnswer().toLowerCase())) {
            ...
            if (search_chr != null && search_chr.getId() != c.getPlayer().getId()) {
                search_chr.getClient().sendPacket(CWvsContext.LieDetector(c.getPlayer().getName(), (byte) 11, (byte) 2, null));
            }
            c.sendPacket(CWvsContext.LieDetector(null, (byte) 10, (byte) 3, null));
            ...
            ld.end();
            c.sendPacket(CWvsContext.LieDetector(null, (byte) 8, (byte) 1, null));
            MapleMap fReturnMap = chr.getMap().getReturnMap();
            chr.changeMap(fReturnMap);
        }
    }

    public static void LieDetectorReset(final MapleClient c, final MapleCharacter user) {
        final MapleLieDetector ld = user.getAntiMacro();
        ld.startLieDetector(ld.getTester(), ld.getLastType() == 0, true);
    }

    public static void HandleNettPyramidMob(LittleEndianAccessor slea, MapleClient c) {
        final int v1 = slea.DecodeInt();
        final int v2 = slea.DecodeInt();
        final MapleMap nMap = c.getPlayer().getMap();
        final MapleMonster nMob = nMap.getMonsterByOid(v2);
        if (nMob != null) {
            nMap.killMonster(nMob);
        }
        final EventManager em = c.getChannelServer().getEventSM().getEventManager("constants_nettpyramid");
        if (em != null) {
            final EventInstanceManager eim = c.getPlayer().getEventInstance();
            if (eim != null) {
                int nLife = Integer.parseInt(em.getProperty("life"));
                em.setProperty("life", (nLife - 1) + "");
                if (nLife > 0) {
                    nMap.broadcastMessage(CNettPyramid.updateLife(nLife - 1));
                } else {
                    nMap.broadcastMessage(CField.showEffect("killing/fail"));
                    for (final MapleCharacter aUser : nMap.getAllCharactersThreadsafe()) {
                        if (aUser != null) {
                            int nWave = Integer.parseInt(em.getProperty("wave"));
                            DueyHandler.addNewItemToDb(4310118, nWave, aUser.getId(), ServerConstants.server_Name_Source, "", true);
                            aUser.getClient().sendPacket(CField.receiveParcel("네트의 피라미드", true));
                            aUser.changeMap(nMap.getForcedReturnMap());
                        }
                    }
                }
            }
    ...
    public static void HandleNettPyramidSkill(LittleEndianAccessor slea, MapleClient c) {
        final MapleCharacter user = c.getPlayer();
        final int nType = slea.DecodeInt();
        final int nItemID = slea.DecodeInt();
        if (user != null) {
            final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            MapleItemInformationProvider.getInstance().getItemEffect(nItemID).applyTo(user);
            user.getMap().broadcastMessage(CWvsContext.getTopMsg(user.getName() + "님이 " + ii.getName(nItemID) + "을 발동하였습니다."));
        }
    }
}
