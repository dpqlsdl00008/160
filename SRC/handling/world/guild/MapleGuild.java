...
public class MapleGuild implements java.io.Serializable {
    ...
    public final void writeToDB(final boolean bDisband) {
        ...
        try {
            ...
            if (!bDisband) {
                ...
                broadcast(GuildPacket.showGuildInfo(null));
                broadcast(NPCPacket.getNPCTalk(2010007, (byte) 0, "길드가 해체되었네..  다시 길드를 만들고 싶으면 언제든지 찾아오게..", "00 00", (byte) 0));
    ...
    public final boolean increaseCapacity(boolean trueMax) {
        ...
        //System.out.println(client.getPlayer().getGuild().capacity);
        //broadcast(NPCPacket.getNPCTalk(2010007, (byte) 0, "\r\n축하하네~ 자네 길드의 인원 수는 이제 #e#b" + this.capacity + "명#k#n으로 늘어났네... 앞으로도 더 많이 성장해서 나를 다시 찾아오도록 하게나...", "00 01", (byte) 0));
        Connection con = null;
        PreparedStatement ps = null;
        try {
            con = DatabaseConnection.getConnection();
    ...
    public boolean purchaseSkill(int skill, String name, int cid) {
        ...
        broadcast(GuildPacket.guildSkillPurchased(id, skill, ourSkill.level, 0, name, name));
        return true;
    }
    ...
    public final void addMemberData(final OutPacket mplew) {
