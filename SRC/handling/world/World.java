...
public class World {
    ...
    public static void runAutoSave() {
        ...
        if (time2 > System.currentTimeMillis()) {
            schedulewait = time2 - System.currentTimeMillis();
        } else {
            schedulewait = time2 + 86400000L - System.currentTimeMillis();
        }
        //System.out.println(schedulewait / 60000+" 분 후에 라이징 스타 작동한당"); // 1381 60
        WorldTimer.getInstance().register(new Runnable() {
            ...
            public void run() {
                ...
                FamilySave();
                //AramiaFireWorks.broadcastStarEvent();
            }
        }, 3600000, schedulewait);
    }

    public static void runTodayTrait() {
        Timer trait = new Timer();
        Calendar datetime = Calendar.getInstance();
        datetime.set(Calendar.HOUR_OF_DAY, 0);
        datetime.set(Calendar.MINUTE, 0);
        datetime.set(Calendar.SECOND, 0);

        trait.schedule(new TraitDailyCheck(), datetime.getTime(), 86400000);
    }

    public static void runAzwanDailyCheck() {
        Timer azwan = new Timer();
        Calendar datetime = Calendar.getInstance();
        datetime.set(Calendar.HOUR_OF_DAY, 0);
        datetime.set(Calendar.MINUTE, 0);
        datetime.set(Calendar.SECOND, 0);

        azwan.schedule(new AzwanDailyCheck(), datetime.getTime(), 86400000);
    }

    public static void runArdentmillReset() {
        Timer ardentmill = new Timer();
        Calendar datetime = Calendar.getInstance();
        datetime.set(Calendar.HOUR_OF_DAY, 0);
        datetime.set(Calendar.MINUTE, 0);
        datetime.set(Calendar.SECOND, 0);

        ardentmill.schedule(new runArdentmillReset(), datetime.getTime(), 86400000);
    ...
    public static class Party {
        ...
        public static Map<Integer, MapleExpedition> expeds = new HashMap<Integer, MapleExpedition>();
            ...
            if (operation == PartyOperation.LEAVE || operation == PartyOperation.EXPEL) {
                ...
                if (chz > 0) {
                    ...
                    if (chr != null) {
                        ...
                        if (oldExped > 0) {
                            chr.getClient().getSession().write(ExpeditionPacket.expeditionMessage(81));
                        }
                ...
                if (ch > 0) {
                    ...
                    if (chr != null) {
                        if (operation == PartyOperation.DISBAND) {
                            chr.setParty(null);
                            if (oldExped > 0) {
                                chr.getClient().getSession().write(ExpeditionPacket.expeditionMessage(82));
                            }
                        } else {
                            chr.setParty(party);
                        }
        ...
        public static Map<Integer, MapleExpedition> getExpeds(int partyid) {
            return expeds;
        }
    ...
    public static class Find {
        ...
        private static final Map<Integer, Integer> accIdToChannel = new ConcurrentHashMap<>();
    ...
    public static class Family {
        ...
        public static void setFamily(int familyid, int seniorid, int junior1, int junior2, int currentrep, int totalrep, int todayrep, int cid) {
            int ch = Find.findChannel(cid);
            if (ch == -1) {
                // System.out.println("ERROR: cannot find player in given channel");
                return;
    ...
    public static void registerRespawn() {
        ...
        for (int i = 0; i < chs.length; i += CHANNELS_PER_THREAD) {
            WorldTimer.getInstance().register(new Respawn(chs, i), 1000);
            WorldTimer.getInstance().register(new DiseaseRunnable(chs, i), 1000);
        }
    }
                            ...
                            if (chr != null) {
                                if (chr.getDiseaseSize() > 0) {
                                    for (MapleDiseaseValueHolder m : chr.getAllDiseases()) {
                                        ...
                                        //if (chr.hasDisease(MonsterSkill.Poison)) {
                                        //int x = chr.getPoison().get(MonsterSkill.Poison).left;
                                        //chr.getMap().broadcastMessage(CField.playerDamaged(chr.getId(), x));
                                        //}
                                    }
                                }

                                //chr.setSoulEnchantEffect();
                                switch (chr.getMapId()) {
                                    case 105200110:
                                    case 105200120: {
                                        chr.setBanBanClock();
                                ...
                                if (GameConstants.isLynn(chr.getJob())) {
                                    chr.updateHealBuff();
                                }
                            }
                        ...
                        if (map.getMobsSize() > 0) {
                            for (MapleMonster mons : map.getAllMonstersThreadsafe()) {
                                if (mons.isAlive() && mons.getStatiSize() > 0) {
                                    for (MonsterTemporaryStatEffect mse : mons.getAllBuffs()) {
                                        if ((mse.getStati() == MonsterTemporaryStat.Burn || mse.getStati() == MonsterTemporaryStat.Poison)) {
        ...
        if (chr.getCooldownSize() > 0) {
            for (MapleCoolDownValueHolder m : chr.getCooldowns()) {
                if (m.startTime + m.length < now) {
                    ...
                    chr.getClient().sendPacket(CUserLocal.skillCooltimeSet(skil, 0));
                }
            }
        }
        if (chr.isAlive()) {
            ...
            //chr.setKannaBuff();
            //}
            if (chr.getFamilyBuffExpEffect() > 0 && chr.getFamilyBuffDropEffect() == 0) {
                if (System.currentTimeMillis() > chr.getFamilyBuffExpDuration()) {
                    chr.setFamilyBuffExpEffect(0);
                    chr.getClient().sendPacket(CWvsContext.FamilyPacket.familyBuff(0, 0, 0, -1));
                    chr.dropMessage(5, "나만의 경험치 1.2배가 종료되었습니다.");
                }
            }
            if (chr.getFamilyBuffDropEffect() > 0 && chr.getFamilyBuffExpEffect() == 0) {
                if (System.currentTimeMillis() > chr.getFamilyBuffDropDuration()) {
                    chr.setFamilyBuffDropEffect(0);
                    chr.getClient().sendPacket(CWvsContext.FamilyPacket.familyBuff(0, 0, 0, -1));
                    chr.dropMessage(5, "나만의 드롭율 1.2배가 종료되었습니다.");
                }
            }
            if (chr.getFamilyBuffExpEffect() > 0 && chr.getFamilyBuffDropEffect() > 0) {
                if (System.currentTimeMillis() > chr.getFamilyBuffDropDuration()) {
            ...
            if (chr.getKeyValue(713419, "useBuff") == 1) {
                int customBuffSkillID = 80001035;
                if (chr.getSkillLevel(customBuffSkillID) <= 0) {
                    chr.changeSingleSkillLevel(SkillFactory.getSkill(customBuffSkillID), (byte) 1, (byte) 1);
                }
            }
            /*
            if (GameConstants.isDemon(chr.getJob())) {
                final Skill effect = SkillFactory.getSkill(31110009);
                final int lvl = chr.getSkillLevel(effect);
                if (lvl > 0) {
                    chr.demonSlayerMaxForce++;
                    if (chr.demonSlayerMaxForce > 3) {
                        chr.demonSlayerMaxForce = 0;
                        int force = effect.getEffect(lvl).getY();
                        chr.addMP(force);
                        //chr.getStat().setMp(chr.getStat().getMp() + force, chr);
                        //chr.updateSingleStat(MapleStat.MP, chr.getStat().getMp());
                        chr.dropMessage(5, "회복 : " + force + " | 총 : " + chr.getStat().getMp());
                    }
                }
            }
             */
    ...
    public static transient ScheduledFuture<?> family;

    public static void FamilySave() {
        if (family == null) {
            family = WorldTimer.getInstance().register(new Runnable() {
                public void run() {
                    MapleFamily.loadAll();
                    System.out.println(ServerConstants.server_Name_Source + "의 모든 패밀리 정보가 갱신 되었습니다.");
                }
            }, 1000 * 60 * 60 * 6);
        }
    }
