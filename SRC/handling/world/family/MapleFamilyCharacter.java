...
public class MapleFamilyCharacter implements java.io.Serializable {
	...
	private int level, id, channel = -1, jobid, familyid, seniorid, currentrep, totalrep, todayrep, junior1, junior2;
	...
	public MapleFamilyCharacter(MapleCharacter c, int fid, int sid, int j1, int j2) {
		...
		totalrep = c.getTotalRep();
                todayrep = c.getTodayRep();
		online = true;
	}

	// or we could just read from the database
	public MapleFamilyCharacter(int _id, int _lv, String _name, int _channel, int _job, int _fid, int _sid, int _jr1, int _jr2, int _crep, int _trep, int trep, boolean _on) {
		level = _lv;
		id = _id;
		name = _name;
		if (_on)
	...
	public void setTodayRep(int tr) {
		this.todayrep = tr;
	}
