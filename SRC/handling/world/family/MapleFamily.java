...
public class MapleFamily implements java.io.Serializable {
    ...
    public MapleFamily(final int fid) {
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT id, name, level, job, seniorid, junior1, junior2, currentrep, totalrep, todayrep FROM characters WHERE familyid = ?", ResultSet.CONCUR_UPDATABLE);
            ps.setInt(1, fid);
            rs = ps.executeQuery();
            while (rs.next()) {
                ...
                members.put(rs.getInt("id"), new MapleFamilyCharacter(rs.getInt("id"), rs.getShort("level"), rs.getString("name"), (byte) -1, rs.getInt("job"), fid, rs.getInt("seniorid"), rs.getInt("junior1"), rs.getInt("junior2"), rs.getInt("currentrep"), rs.getInt("totalrep"), rs.getInt("todayrep"), false));
            }
            rs.close();
            ps.close();
        ...
        for (MapleFamilyCharacter mgc : members.values()) {
            if (cids == null || cids.contains(mgc.getId())) {
                if (bcop == FCOp.DISBAND) {
                    if (mgc.isOnline()) {
                        World.Family.setFamily(0, 0, 0, 0, mgc.getCurrentRep(), mgc.getTotalRep(), mgc.getTodayRep(), mgc.getId());
                    } else {
                        setOfflineFamilyStatus(0, 0, 0, 0, mgc.getCurrentRep(), mgc.getTotalRep(), mgc.getTodayRep(), mgc.getId());
                    }
    ...
    public final int setRep(final int cid, int addrep, final int oldLevel, final String oldName) {
        ...
        if (mgc != null && mgc.getFamilyId() == id) {
            ...
            if (mgc.isOnline()) {
                ...
                World.Family.setFamily(id, mgc.getSeniorId(), mgc.getJunior1(), mgc.getJunior2(), mgc.getCurrentRep() + addrep, mgc.getTotalRep() + addrep, mgc.getTodayRep() + addrep, mgc.getId());
            } else {
                mgc.setCurrentRep(mgc.getCurrentRep() + addrep);
                mgc.setTotalRep(mgc.getTotalRep() + addrep);
                mgc.setTodayRep(mgc.getTodayRep() + addrep);
                setOfflineFamilyStatus(id, mgc.getSeniorId(), mgc.getJunior1(), mgc.getJunior2(), mgc.getCurrentRep(), mgc.getTotalRep(), mgc.getTodayRep(), mgc.getId());
            }
    ...
    public static void setOfflineFamilyStatus(int familyid, int seniorid, int junior1, int junior2, int currentrep, int totalrep, int todayrep, int cid) {
        Connection con = null;
        PreparedStatement ps = null;
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("UPDATE characters SET familyid = ?, seniorid = ?, junior1 = ?, junior2 = ?, currentrep = ?, totalrep = ?, todayrep = ? WHERE id = ?");
    ...
    public static void mergeFamily(MapleFamily newfam, MapleFamily oldfam, int leaderId) {
        for (MapleFamilyCharacter mgc : oldfam.members.values()) {
            ...
            if (mgc.isOnline()) {
                World.Family.setFamily(newfam.getId(), mgc.getSeniorId(), mgc.getJunior1(), mgc.getJunior2(), mgc.getCurrentRep(), mgc.getTotalRep(), mgc.getTodayRep(), mgc.getId());
            } else {
                setOfflineFamilyStatus(newfam.getId(), mgc.getSeniorId(), mgc.getJunior1(), mgc.getJunior2(), mgc.getCurrentRep(), mgc.getTotalRep(), mgc.getTodayRep(), mgc.getId());
            }
    ...
    public boolean splitFamily(int splitId, MapleFamilyCharacter def) {
        ...
        try {
            ...
            for (MapleFamilyCharacter mgc : all) {
                ...
                setOfflineFamilyStatus(newId, mgc.getSeniorId(), mgc.getJunior1(), mgc.getJunior2(), mgc.getCurrentRep(), mgc.getTotalRep(), mgc.getTodayRep(), mgc.getId());
            ...
            for (MapleFamilyCharacter mgc : all) {
                if (mgc.isOnline()) { //NOW we change the char info
                    World.Family.setFamily(newId, mgc.getSeniorId(), mgc.getJunior1(), mgc.getJunior2(), mgc.getCurrentRep(), mgc.getTotalRep(), mgc.getTodayRep(), mgc.getId());
                }
                newfam.setOnline(mgc.getId(), mgc.isOnline(), mgc.getChannel());
            }
        } finally {
